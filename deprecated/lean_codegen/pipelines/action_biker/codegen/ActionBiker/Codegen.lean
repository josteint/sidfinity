/-
  ActionBiker.Codegen — Player codegen for Rob Hubbard's ActionBiker on the Run,
  ...
  ActionBiker-specific, are flagged inline:
...
import ActionBiker.SID
import ActionBiker.Asm6502
import ActionBiker.PSIDFile
import ActionBiker.USF
import ActionBiker.Constants

namespace ActionBikerNS
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig ActionBiker: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for ActionBiker. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in ActionBiker's V3 first-pattern
  ...
  -- (must disassemble ActionBiker's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the ActionBiker binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the ActionBiker binary at $84E5..$84E7
...
--  not applicable in the cloned ActionBiker codegen. SidgenActionBikerMain.lean
--  provides its own main using `action_bikerV3` from ActionBikerV3.lean.)
