/-
  DragonsLairPartIi.Codegen — Player codegen for Rob Hubbard's DragonsLairPartIi on the Run,
  ...
  DragonsLairPartIi-specific, are flagged inline:
...
import DragonsLairPartIi.SID
import DragonsLairPartIi.Asm6502
import DragonsLairPartIi.PSIDFile
import DragonsLairPartIi.USF
import DragonsLairPartIi.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig DragonsLairPartIi: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for DragonsLairPartIi. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in DragonsLairPartIi's V3 first-pattern
  ...
  -- (must disassemble DragonsLairPartIi's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the DragonsLairPartIi binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the DragonsLairPartIi binary at $84E5..$84E7
...
--  not applicable in the cloned DragonsLairPartIi codegen. SidgenDragonsLairPartIiMain.lean
--  provides its own main using `dragons_lair_part_iiV3` from DragonsLairPartIiV3.lean.)
