"""Generate USF v3 BattleOfBritain on the Run data as a Lean file.
...
Discovered freq_table_addr for BattleOfBritain: $8400 (via src/sidxray/discover.py).
"""
...
BATTLE_OF_BRITAIN_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Battle_of_Britain.sid'
BATTLE_OF_BRITAIN_FT_BASE = 0x8326  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from BattleOfBritain (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=BATTLE_OF_BRITAIN_SID,
            ft_base=BATTLE_OF_BRITAIN_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 BattleOfBritain on the Run data",
        f"-- Subtunes: {subtune_indices} (0-indexed; PSID subtunes "
        f"{[s + 1 for s in subtune_indices]})",
        "import BattleOfBritain.USF",
        "",
    ]

    out.append(
        f"def battle_of_britainV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for BattleOfBritain. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). BattleOfBritain doesn't do this; keeping them would
    #     overwrite BattleOfBritain's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def battle_of_britainV3 : USFSong := {{
  freqTable := battle_of_britainV3FreqTable
  ...
  playRate := .vbi
  engineQuirks := {quirks}
  title := "BattleOfBritain on the Run"
  author := "Rob Hubbard"
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/battle_of_britain/codegen/BattleOfBritain/SongData.lean'
    )
