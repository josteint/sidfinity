...
Produces the ACME-flavoured listing that `pipelines/battle_of_britain/
...
Used by hand to seed `pipelines/battle_of_britain/disassembly.s`.
Reusable for any PSID — pass the .sid path on the command line.

Usage:
    python -m pipelines.battle_of_britain.extract.disasm \\
...
def main() -> None:
    if len(sys.argv) < 2:
        sys.stderr.write(
            'usage: python -m pipelines.battle_of_britain.extract.disasm '
            '<path.sid>\n')
        sys.exit(2)
    path = sys.argv[1]
