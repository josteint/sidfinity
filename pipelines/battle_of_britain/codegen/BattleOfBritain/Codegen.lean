/-
  BattleOfBritain.Codegen — Player codegen for Rob Hubbard's BattleOfBritain on the Run,
  ...
  BattleOfBritain-specific, are flagged inline:
...
import BattleOfBritain.SID
import BattleOfBritain.Asm6502
import BattleOfBritain.PSIDFile
import BattleOfBritain.USF
import BattleOfBritain.Constants

namespace BattleOfBritainNS
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig BattleOfBritain: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for BattleOfBritain. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in BattleOfBritain's V3 first-pattern
  ...
  -- (must disassemble BattleOfBritain's player to find what gets written to
...
--  not applicable in the cloned BattleOfBritain codegen. SidgenBattleOfBritainMain.lean
--  provides its own main using `battle_of_britainV3` from BattleOfBritainV3.lean.)

end BattleOfBritainNS
