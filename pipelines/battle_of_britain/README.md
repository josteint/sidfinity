# BattleOfBritain on the Run pipeline

End-to-end rebuild of Rob Hubbard's *BattleOfBritain on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for BattleOfBritain's engine quirks
...
explanation. The BattleOfBritain-specific differences are inside the codegen:

| File | BattleOfBritain-only addition |
|---|---|
| `codegen/BattleOfBritain/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/BattleOfBritain/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.battle_of_britain.extract.emit_usf            # subtune 0 only
python -m pipelines.battle_of_britain.extract.emit_usf 0,1,2       # all three music tracks
```
...
invariant safe while BattleOfBritain was being developed. The two pipelines can
