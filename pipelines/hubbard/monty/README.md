...
End-to-end rebuild of Rob Hubbard's *Monty on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for Monty's engine quirks
...
explanation. The Monty-specific differences are inside the codegen:

| File | Monty-only addition |
|---|---|
| `codegen/Monty/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/Monty/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.hubbard.monty.extract.emit_usf            # subtune 0 only
python -m pipelines.hubbard.monty.extract.emit_usf 0,1,2       # all three music tracks
```
...
# → pipelines/hubbard/monty/build/monty.sid
```
...
python src/writelog_grade.py \
    hvsc85/MUSICIANS/H/Hubbard_Rob/Monty_on_the_Run.sid \
    pipelines/hubbard/monty/build/monty.sid
...
invariant safe while Monty was being developed. The two pipelines can
