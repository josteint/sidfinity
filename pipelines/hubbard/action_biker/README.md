...
explanation. The ActionBiker-specific differences are inside the codegen:

| File | ActionBiker-only addition |
|---|---|
| `codegen/ActionBiker/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/ActionBiker/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.hubbard.action_biker.extract.emit_usf            # subtune 0 only
python -m pipelines.hubbard.action_biker.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_action_biker
./.lake/build/bin/sidgen_action_biker
```
...
python src/writelog_grade.py \
    hvsc85/MUSICIANS/H/Hubbard_Rob/Action_Biker.sid \
    action_biker.sid
...
invariant safe while ActionBiker was being developed. The two pipelines can
