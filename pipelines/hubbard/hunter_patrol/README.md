...
| Build | end-to-end clean: `python -m pipelines.hubbard.hunter_patrol.extract` → `lake build sidgen_hunter_patrol` → SID at `pipelines/hubbard/hunter_patrol/build/hunter_patrol.sid` |
...
```bash
python -m pipelines.hubbard.hunter_patrol.extract              # subtune 0 (the only one)
```
