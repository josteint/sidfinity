...
End-to-end rebuild of Rob Hubbard's *Chimera on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for Chimera's engine quirks
...
explanation. The Chimera-specific differences are inside the codegen:

| File | Chimera-only addition |
|---|---|
| `codegen/Chimera/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/Chimera/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.hubbard.chimera.extract.emit_usf            # subtune 0 only
python -m pipelines.hubbard.chimera.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_chimera
./.lake/build/bin/sidgen_chimera
```
...
python src/writelog_grade.py \
    hvsc84/MUSICIANS/H/Hubbard_Rob/Chimera.sid \
    chimera.sid
...
invariant safe while Chimera was being developed. The two pipelines can
