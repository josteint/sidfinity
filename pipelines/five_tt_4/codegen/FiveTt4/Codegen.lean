/-
  FiveTt4.Codegen — Player codegen for Rob Hubbard's FiveTt4 on the Run,
  ...
  FiveTt4-specific, are flagged inline:
...
import FiveTt4.SID
import FiveTt4.Asm6502
import FiveTt4.PSIDFile
import FiveTt4.USF
import FiveTt4.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig FiveTt4: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for FiveTt4. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in FiveTt4's V3 first-pattern
  ...
  -- (must disassemble FiveTt4's player to find what gets written to
...
def generateSID (song : USFSong) (baseAddr : UInt16 := 0x1000) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the FiveTt4 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the FiveTt4 binary at $84E5..$84E7
...
--  not applicable in the cloned FiveTt4 codegen. SidgenFiveTt4Main.lean
--  provides its own main using `five_tt_4V3` from FiveTt4V3.lean.)
