/-
  BumpSetSpike.Codegen — Player codegen for Rob Hubbard's BumpSetSpike, consuming
  a `USFSong` and emitting a `ByteArray` PSID. Locked invariant: the
  resulting `bump_set_spike.sid` has md5 `1964b77e8b542a5187fdd0a6db2d0186` and
...
import BumpSetSpike.SID
import BumpSetSpike.Asm6502
import BumpSetSpike.PSIDFile
import BumpSetSpike.USF
import BumpSetSpike.Constants
...
def emitSustainEffects (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- For BumpSetSpike tempo=3: skip until countdown <= durationFrames - tempo = durationFrames - 3.
