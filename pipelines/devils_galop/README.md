# DevilsGalop on the Run pipeline

End-to-end rebuild of Rob Hubbard's *DevilsGalop on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for DevilsGalop's engine quirks
...
explanation. The DevilsGalop-specific differences are inside the codegen:

| File | DevilsGalop-only addition |
|---|---|
| `codegen/DevilsGalop/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/DevilsGalop/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.devils_galop.extract.emit_usf            # subtune 0 only
python -m pipelines.devils_galop.extract.emit_usf 0,1,2       # all three music tracks
```
...
invariant safe while DevilsGalop was being developed. The two pipelines can
