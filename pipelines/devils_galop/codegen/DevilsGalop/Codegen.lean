/-
  DevilsGalop.Codegen — Player codegen for Rob Hubbard's DevilsGalop on the Run,
  ...
  DevilsGalop-specific, are flagged inline:
...
import DevilsGalop.SID
import DevilsGalop.Asm6502
import DevilsGalop.PSIDFile
import DevilsGalop.USF
import DevilsGalop.Constants

namespace DevilsGalopNS
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig DevilsGalop: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for DevilsGalop. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in DevilsGalop's V3 first-pattern
  ...
  -- (must disassemble DevilsGalop's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the DevilsGalop binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the DevilsGalop binary at $84E5..$84E7
...
--  not applicable in the cloned DevilsGalop codegen. SidgenDevilsGalopMain.lean
--  provides its own main using `devils_galopV3` from DevilsGalopV3.lean.)
