...
structure USFEngineQuirks where
  ...
  -- Per-voice initial PWM state, extracted from the original binary's
  -- pulsedelay/pulsedir bytes. Was hardcoded in Codegen.lean per-clone;
  -- now data-driven so one Codegen lib can serve multiple subs.
  pwmPeriodInit      : List UInt8 := [0, 0, 0]   -- v_pwperiod[V1..V3] at song start
  pwmDirInit         : List UInt8 := [0, 0, 0]   -- v_pwdir[V1..V3] at song start
  deriving Repr
