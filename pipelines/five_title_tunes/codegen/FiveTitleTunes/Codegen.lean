/-
  FiveTitleTunes.Codegen — Player codegen for Rob Hubbard's FiveTitleTunes on the Run,
  ...
  FiveTitleTunes-specific, are flagged inline:
...
import FiveTitleTunes.SID
import FiveTitleTunes.Asm6502
import FiveTitleTunes.PSIDFile
import FiveTitleTunes.USF
import FiveTitleTunes.Constants
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in FiveTitleTunes's V3 first-pattern
  ...
  -- (must disassemble FiveTitleTunes's player to find what gets written to
...
def generateSID (song : USFSong) (baseAddr : UInt16 := 0x1000) (debug : Bool := false) : Bytes := Id.run do
  let base : UInt16 := baseAddr
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir / pulsedelay initial bytes come from `song.engineQuirks`
  -- (extracted from the original binary's data section by emit_usf.py).
  -- Default `[0,0,0]` matches Commando; per-sub-binary values vary.
  cb := cb.label "v_pwdir"
  cb := cb.emitData song.engineQuirks.pwmDirInit
  cb := cb.label "v_pwperiod"
  cb := cb.emitData song.engineQuirks.pwmPeriodInit
...
--  not applicable in the cloned FiveTitleTunes codegen. SidgenFiveTt0Main.lean
--  provides its own main using `sub0V3` from FiveTt0V3.lean.)
