"""Smoke tests for the FiveTitleTunes on the Run extract pipeline.

Run from repo root:  pytest pipelines/five_title_tunes/tests/
"""
...
def test_extract_returns_song_with_expected_shape() -> None:
    ...
    assert len(song.instruments) == 20      # FiveTitleTunes has 20 distinct instruments
    assert song.score.tempo > 0
    assert len(song.score.voices) == 3


def test_skydive_instruments_are_detected() -> None:
    """FiveTitleTunes's instruments 10, 12, 13 have fx_flags bit 1 set (skydive)."""
    song = extract(subtune=0)
...
def test_pw_bounds_are_hubbard_hardcoded() -> None:
    ...
    every FiveTitleTunes extraction must default to those values."""
    song = extract(subtune=0)
