"""Generate USF v3 FiveTitleTunes on the Run data as a Lean file.
...
Discovered freq_table_addr for FiveTitleTunes: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    FiveTitleTunes does NOT do this — its pitch 104 is a real freq lookup
...
FIVE_TITLE_TUNES_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/5_Title_Tunes.sid'
FIVE_TITLE_TUNES_FT_BASE = 0x0F6A  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from FiveTitleTunes (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=FIVE_TITLE_TUNES_SID,
            ft_base=FIVE_TITLE_TUNES_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def five_title_tunesV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for FiveTitleTunes. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). FiveTitleTunes doesn't do this; keeping them would
    #     overwrite FiveTitleTunes's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def five_title_tunesV3 : USFSong := {{
  freqTable := five_title_tunesV3FreqTable
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/five_title_tunes/codegen/FiveTitleTunes/SongData.lean'
    )
