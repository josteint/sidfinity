"""Generate USF v3 LastV8 on the Run data as a Lean file.
...
Discovered freq_table_addr for LastV8: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    LastV8 does NOT do this — its pitch 104 is a real freq lookup
...
LAST_V8_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Last_V8.sid'
LAST_V8_FT_BASE = 0x843B  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from LastV8 (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=LAST_V8_SID,
            ft_base=LAST_V8_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def last_v8V3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for LastV8. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). LastV8 doesn't do this; keeping them would
    #     overwrite LastV8's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def last_v8V3 : USFSong := {{
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/last_v8/codegen/LastV8/SongData.lean'
    )
