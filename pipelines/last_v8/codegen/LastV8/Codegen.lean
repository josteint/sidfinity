/-
  LastV8.Codegen — Player codegen for Rob Hubbard's LastV8 on the Run,
  ...
  LastV8-specific, are flagged inline:
...
import LastV8.SID
import LastV8.Asm6502
import LastV8.PSIDFile
import LastV8.USF
import LastV8.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig LastV8: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for LastV8. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in LastV8's V3 first-pattern
  ...
  -- (must disassemble LastV8's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the LastV8 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the LastV8 binary at $84E5..$84E7
...
--  not applicable in the cloned LastV8 codegen. SidgenLastV8Main.lean
--  provides its own main using `last_v8V3` from LastV8V3.lean.)
