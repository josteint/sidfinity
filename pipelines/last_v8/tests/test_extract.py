"""Smoke tests for the LastV8 on the Run extract pipeline.

Run from repo root:  pytest pipelines/last_v8/tests/
"""
from __future__ import annotations

from pipelines.last_v8.extract.engine_model import extract
...
def test_extract_returns_song_with_expected_shape() -> None:
    ...
    assert len(song.instruments) == 20      # LastV8 has 20 distinct instruments
    assert song.score.tempo > 0
    assert len(song.score.voices) == 3


def test_skydive_instruments_are_detected() -> None:
    """LastV8's instruments 10, 12, 13 have fx_flags bit 1 set (skydive)."""
    song = extract(subtune=0)
...
def test_pw_bounds_are_hubbard_hardcoded() -> None:
    ...
    every LastV8 extraction must default to those values."""
    song = extract(subtune=0)
