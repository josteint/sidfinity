# LastV8 on the Run pipeline

End-to-end rebuild of Rob Hubbard's *LastV8 on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for LastV8's engine quirks
...
explanation. The LastV8-specific differences are inside the codegen:

| File | LastV8-only addition |
|---|---|
| `codegen/LastV8/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/LastV8/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.last_v8.extract.emit_usf            # subtune 0 only
python -m pipelines.last_v8.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_last_v8
...
invariant safe while LastV8 was being developed. The two pipelines can
