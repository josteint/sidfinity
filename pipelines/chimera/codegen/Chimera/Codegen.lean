/-
  Chimera.Codegen — Player codegen for Rob Hubbard's Chimera on the Run,
  ...
  Chimera-specific, are flagged inline:
...
import Chimera.SID
import Chimera.Asm6502
import Chimera.PSIDFile
import Chimera.USF
import Chimera.Constants
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in Chimera's V3 first-pattern
  ...
  -- (must disassemble Chimera's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the Chimera binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the Chimera binary at $84E5..$84E7
...
--  not applicable in the cloned Chimera codegen. SidgenChimeraMain.lean
--  provides its own main using `chimeraV3` from ChimeraV3.lean.)
