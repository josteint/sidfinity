"""Generate USF v3 Chimera on the Run data as a Lean file.
...
Discovered freq_table_addr for Chimera: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    Chimera does NOT do this — its pitch 104 is a real freq lookup
...
CHIMERA_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Chimera.sid'
CHIMERA_FT_BASE = 0xC567  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from Chimera (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=CHIMERA_SID,
            ft_base=CHIMERA_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 Chimera on the Run data",
    ...
    out.append(
        f"def chimeraV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for Chimera. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). Chimera doesn't do this; keeping them would
    #     overwrite Chimera's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def chimeraV3 : USFSong := {{
  freqTable := chimeraV3FreqTable
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/chimera/codegen/Chimera/SongData.lean'
    )
