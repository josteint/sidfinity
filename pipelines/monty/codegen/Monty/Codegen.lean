...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig Monty: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for Monty. Commando used cmp_imm 2; the
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the Monty binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the Monty binary at $84E5..$84E7
