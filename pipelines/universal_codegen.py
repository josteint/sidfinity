...
def emit_sid(usf: UsfFile, usf_dir: str | None = None) -> bytes:
    ...
    if shape == 'companion':
        ...
        # Atomic + pair + cmd-stream USFs (henrys + bowden + yes_tune +
        # clever_music) now go through `pipelines.composer`. If they
        # reach here, the composer rejected the USF for some other
        # reason — raise rather than silently fall through.
        raise NotImplementedError(
