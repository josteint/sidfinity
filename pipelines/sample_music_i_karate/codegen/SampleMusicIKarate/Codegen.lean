...
import SampleMusicIKarate.SID
import SampleMusicIKarate.Asm6502
import SampleMusicIKarate.PSIDFile
import SampleMusicIKarate.USF
import SampleMusicIKarate.Constants

namespace SampleMusicIKarateNS
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig SampleMusicIKarate: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for SampleMusicIKarate. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in SampleMusicIKarate's V3 first-pattern
  ...
  -- (must disassemble SampleMusicIKarate's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the SampleMusicIKarate binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the SampleMusicIKarate binary at $84E5..$84E7
...
--  not applicable in the cloned SampleMusicIKarate codegen. SidgenSampleMusicIKarateMain.lean
--  provides its own main using `sample_music_i_karateV3` from SampleMusicIKarateV3.lean.)

end SampleMusicIKarateNS
