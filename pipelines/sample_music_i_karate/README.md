# SampleMusicIKarate on the Run pipeline

End-to-end rebuild of Rob Hubbard's *SampleMusicIKarate on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for SampleMusicIKarate's engine quirks
...
explanation. The SampleMusicIKarate-specific differences are inside the codegen:

| File | SampleMusicIKarate-only addition |
|---|---|
| `codegen/SampleMusicIKarate/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/SampleMusicIKarate/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.sample_music_i_karate.extract.emit_usf            # subtune 0 only
python -m pipelines.sample_music_i_karate.extract.emit_usf 0,1,2       # all three music tracks
```
...
invariant safe while SampleMusicIKarate was being developed. The two pipelines can
