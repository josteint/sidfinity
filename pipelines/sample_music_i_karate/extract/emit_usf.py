...
Discovered freq_table_addr for SampleMusicIKarate: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    SampleMusicIKarate does NOT do this — its pitch 104 is a real freq lookup
...
SAMPLE_MUSIC_I_KARATE_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Sample_Music_from_I_Karate.sid'
SAMPLE_MUSIC_I_KARATE_FT_BASE = 0x13FC  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from SampleMusicIKarate (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=SAMPLE_MUSIC_I_KARATE_SID,
            ft_base=SAMPLE_MUSIC_I_KARATE_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def sample_music_i_karateV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for SampleMusicIKarate. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). SampleMusicIKarate doesn't do this; keeping them would
    #     overwrite SampleMusicIKarate's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def sample_music_i_karateV3 : USFSong := {{
  ...
  released := "1985 Rob Hubbard"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/sample_music_i_karate/codegen/SampleMusicIKarate/SongData.lean'
    )
