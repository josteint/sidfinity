"""Generate USF v3 Rasputin on the Run data as a Lean file.
...
Discovered freq_table_addr for Rasputin: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    Rasputin does NOT do this — its pitch 104 is a real freq lookup
...
RASPUTIN_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Rasputin.sid'
RASPUTIN_FT_BASE = 0xC448  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from Rasputin (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=RASPUTIN_SID,
            ft_base=RASPUTIN_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def rasputinV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for Rasputin. Same Hubbard engine as Commando, so:
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/rasputin/codegen/Rasputin/SongData.lean'
    )
