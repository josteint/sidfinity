"""Smoke tests for the Rasputin on the Run extract pipeline.

Run from repo root:  pytest pipelines/rasputin/tests/
"""
from __future__ import annotations

from pipelines.rasputin.extract.engine_model import extract
...
def test_extract_returns_song_with_expected_shape() -> None:
    ...
    assert len(song.instruments) == 20      # Rasputin has 20 distinct instruments
    assert song.score.tempo > 0
    assert len(song.score.voices) == 3


def test_skydive_instruments_are_detected() -> None:
    """Rasputin's instruments 10, 12, 13 have fx_flags bit 1 set (skydive)."""
    song = extract(subtune=0)
...
def test_pw_bounds_are_hubbard_hardcoded() -> None:
    ...
    every Rasputin extraction must default to those values."""
    song = extract(subtune=0)
