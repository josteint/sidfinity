/-
  Rasputin.Codegen — Player codegen for Rob Hubbard's Rasputin on the Run,
  ...
  Rasputin-specific, are flagged inline:
...
import Rasputin.SID
import Rasputin.Asm6502
import Rasputin.PSIDFile
import Rasputin.USF
import Rasputin.Constants

namespace RasputinNS
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig Rasputin: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for Rasputin. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in Rasputin's V3 first-pattern
  ...
  -- (must disassemble Rasputin's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the Rasputin binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the Rasputin binary at $84E5..$84E7
...
--  not applicable in the cloned Rasputin codegen. SidgenRasputinMain.lean
--  provides its own main using `rasputinV3` from RasputinV3.lean.)
