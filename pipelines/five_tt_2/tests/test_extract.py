"""Smoke tests for the FiveTt2 on the Run extract pipeline.

Run from repo root:  pytest pipelines/five_tt_2/tests/
"""
from __future__ import annotations

from pipelines.five_tt_2.extract.engine_model import extract
...
def test_extract_returns_song_with_expected_shape() -> None:
    ...
    assert len(song.instruments) == 20      # FiveTt2 has 20 distinct instruments
    assert song.score.tempo > 0
    assert len(song.score.voices) == 3


def test_skydive_instruments_are_detected() -> None:
    """FiveTt2's instruments 10, 12, 13 have fx_flags bit 1 set (skydive)."""
    song = extract(subtune=0)
...
def test_pw_bounds_are_hubbard_hardcoded() -> None:
    ...
    every FiveTt2 extraction must default to those values."""
    song = extract(subtune=0)
