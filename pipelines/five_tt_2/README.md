# FiveTt2 on the Run pipeline

End-to-end rebuild of Rob Hubbard's *FiveTt2 on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for FiveTt2's engine quirks
...
explanation. The FiveTt2-specific differences are inside the codegen:

| File | FiveTt2-only addition |
|---|---|
| `codegen/FiveTt2/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/FiveTt2/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.five_tt_2.extract.emit_usf            # subtune 0 only
python -m pipelines.five_tt_2.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_five_tt_2
./.lake/build/bin/sidgen_five_tt_2
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/sub_2.sid \
    five_tt_2.sid
...
invariant safe while FiveTt2 was being developed. The two pipelines can
