/-
  FiveTt2.Codegen — Player codegen for Rob Hubbard's FiveTt2 on the Run,
  ...
  FiveTt2-specific, are flagged inline:
...
import FiveTt2.SID
import FiveTt2.Asm6502
import FiveTt2.PSIDFile
import FiveTt2.USF
import FiveTt2.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig FiveTt2: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for FiveTt2. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in FiveTt2's V3 first-pattern
  ...
  -- (must disassemble FiveTt2's player to find what gets written to
...
def generateSID (song : USFSong) (baseAddr : UInt16 := 0x1000) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the FiveTt2 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the FiveTt2 binary at $84E5..$84E7
...
--  not applicable in the cloned FiveTt2 codegen. SidgenFiveTt2Main.lean
--  provides its own main using `five_tt_2V3` from FiveTt2V3.lean.)
