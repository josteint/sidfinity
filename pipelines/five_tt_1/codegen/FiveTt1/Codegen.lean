/-
  FiveTt1.Codegen — Player codegen for Rob Hubbard's FiveTt1 on the Run,
  ...
  FiveTt1-specific, are flagged inline:
...
import FiveTt1.SID
import FiveTt1.Asm6502
import FiveTt1.PSIDFile
import FiveTt1.USF
import FiveTt1.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig FiveTt1: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for FiveTt1. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in FiveTt1's V3 first-pattern
  ...
  -- (must disassemble FiveTt1's player to find what gets written to
...
def generateSID (song : USFSong) (baseAddr : UInt16 := 0x1000) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the FiveTt1 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the FiveTt1 binary at $84E5..$84E7
...
--  not applicable in the cloned FiveTt1 codegen. SidgenFiveTt1Main.lean
--  provides its own main using `five_tt_1V3` from FiveTt1V3.lean.)
