# FiveTt1 on the Run pipeline

End-to-end rebuild of Rob Hubbard's *FiveTt1 on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for FiveTt1's engine quirks
...
explanation. The FiveTt1-specific differences are inside the codegen:

| File | FiveTt1-only addition |
|---|---|
| `codegen/FiveTt1/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/FiveTt1/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.five_tt_1.extract.emit_usf            # subtune 0 only
python -m pipelines.five_tt_1.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_five_tt_1
./.lake/build/bin/sidgen_five_tt_1
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/sub_1.sid \
    five_tt_1.sid
...
invariant safe while FiveTt1 was being developed. The two pipelines can
