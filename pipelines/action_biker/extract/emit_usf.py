"""Generate USF v3 ActionBiker on the Run data as a Lean file.
...
Discovered freq_table_addr for ActionBiker: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    ActionBiker does NOT do this — its pitch 104 is a real freq lookup
...
ACTION_BIKER_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Action_Biker.sid'
ACTION_BIKER_FT_BASE = 0xC2FC  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from ActionBiker (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=ACTION_BIKER_SID,
            ft_base=ACTION_BIKER_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def action_bikerV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for ActionBiker. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). ActionBiker doesn't do this; keeping them would
    #     overwrite ActionBiker's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def action_bikerV3 : USFSong := {{
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/action_biker/codegen/ActionBiker/SongData.lean'
    )
