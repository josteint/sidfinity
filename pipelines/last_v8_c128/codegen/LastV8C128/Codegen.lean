/-
  LastV8C128.Codegen — Player codegen for Rob Hubbard's LastV8C128 on the Run,
  ...
  LastV8C128-specific, are flagged inline:
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig LastV8C128: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for LastV8C128. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in LastV8C128's V3 first-pattern
  ...
  -- (must disassemble LastV8C128's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the LastV8C128 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the LastV8C128 binary at $84E5..$84E7
...
--  not applicable in the cloned LastV8C128 codegen. SidgenLastV8C128Main.lean
--  provides its own main using `last_v8_c128V3` from LastV8C128V3.lean.)
