# LastV8C128 on the Run pipeline

End-to-end rebuild of Rob Hubbard's *LastV8C128 on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for LastV8C128's engine quirks
...
explanation. The LastV8C128-specific differences are inside the codegen:

| File | LastV8C128-only addition |
|---|---|
| `codegen/LastV8C128/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/LastV8C128/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.last_v8_c128.extract.emit_usf            # subtune 0 only
python -m pipelines.last_v8_c128.extract.emit_usf 0,1,2       # all three music tracks
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/Last_V8_C128_version.sid \
    last_v8_c128.sid
...
invariant safe while LastV8C128 was being developed. The two pipelines can
