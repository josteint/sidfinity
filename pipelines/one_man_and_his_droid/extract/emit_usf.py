...
def gen_freq_table(T: list[int]) -> str:
    ...
    OneManAndHisDroid does NOT do this — its pitch 104 is a real freq lookup
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from OneManAndHisDroid (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=ONE_MAN_AND_HIS_DROID_SID,
            ft_base=ONE_MAN_AND_HIS_DROID_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def one_man_and_his_droidV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for OneManAndHisDroid. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). OneManAndHisDroid doesn't do this; keeping them would
    #     overwrite OneManAndHisDroid's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def one_man_and_his_droidV3 : USFSong := {{
  freqTable := one_man_and_his_droidV3FreqTable
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/one_man_and_his_droid/codegen/OneManAndHisDroid/SongData.lean'
    )
