# OneManAndHisDroid on the Run pipeline

End-to-end rebuild of Rob Hubbard's *OneManAndHisDroid on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for OneManAndHisDroid's engine quirks
...
explanation. The OneManAndHisDroid-specific differences are inside the codegen:

| File | OneManAndHisDroid-only addition |
|---|---|
| `codegen/OneManAndHisDroid/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/OneManAndHisDroid/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.one_man_and_his_droid.extract.emit_usf            # subtune 0 only
python -m pipelines.one_man_and_his_droid.extract.emit_usf 0,1,2       # all three music tracks
```
...
invariant safe while OneManAndHisDroid was being developed. The two pipelines can
