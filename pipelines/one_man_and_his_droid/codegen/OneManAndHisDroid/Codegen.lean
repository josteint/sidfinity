/-
  OneManAndHisDroid.Codegen — Player codegen for Rob Hubbard's OneManAndHisDroid on the Run,
  ...
  OneManAndHisDroid-specific, are flagged inline:
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig OneManAndHisDroid: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for OneManAndHisDroid. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in OneManAndHisDroid's V3 first-pattern
  ...
  -- (must disassemble OneManAndHisDroid's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the OneManAndHisDroid binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the OneManAndHisDroid binary at $84E5..$84E7
...
--  not applicable in the cloned OneManAndHisDroid codegen. SidgenOneManAndHisDroidMain.lean
--  provides its own main using `one_man_and_his_droidV3` from OneManAndHisDroidV3.lean.)

end OneManAndHisDroidNS
