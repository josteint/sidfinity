...
def gen_freq_table(T: list[int]) -> str:
    ...
    HunterPatrol does NOT do this — its pitch 104 is a real freq lookup
...
HUNTER_PATROL_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Hunter_Patrol.sid'
HUNTER_PATROL_FT_BASE = 0xA32D  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from HunterPatrol (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=HUNTER_PATROL_SID,
            ft_base=HUNTER_PATROL_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def hunter_patrolV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for HunterPatrol. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). HunterPatrol doesn't do this; keeping them would
    #     overwrite HunterPatrol's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def hunter_patrolV3 : USFSong := {{
  freqTable := hunter_patrolV3FreqTable
  ...
  released := "1985 Mastertronic"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/hunter_patrol/codegen/HunterPatrol/SongData.lean'
    )
