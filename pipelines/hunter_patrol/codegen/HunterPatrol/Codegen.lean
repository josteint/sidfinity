...
import HunterPatrol.SID
import HunterPatrol.Asm6502
import HunterPatrol.PSIDFile
import HunterPatrol.USF
import HunterPatrol.Constants

namespace HunterPatrolNS
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in HunterPatrol's V3 first-pattern
  ...
  -- (must disassemble HunterPatrol's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the HunterPatrol binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the HunterPatrol binary at $84E5..$84E7
...
--  not applicable in the cloned HunterPatrol codegen. SidgenHunterPatrolMain.lean
--  provides its own main using `hunter_patrolV3` from HunterPatrolV3.lean.)
