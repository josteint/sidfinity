# CrazyComets on the Run pipeline

End-to-end rebuild of Rob Hubbard's *CrazyComets on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for CrazyComets's engine quirks
...
explanation. The CrazyComets-specific differences are inside the codegen:

| File | CrazyComets-only addition |
|---|---|
| `codegen/CrazyComets/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/CrazyComets/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.crazy_comets.extract.emit_usf            # subtune 0 only
python -m pipelines.crazy_comets.extract.emit_usf 0,1,2       # all three music tracks
```
...
invariant safe while CrazyComets was being developed. The two pipelines can
