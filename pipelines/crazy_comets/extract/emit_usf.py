"""Generate USF v3 CrazyComets on the Run data as a Lean file.
...
Discovered freq_table_addr for CrazyComets: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    CrazyComets does NOT do this — its pitch 104 is a real freq lookup
...
CRAZY_COMETS_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Crazy_Comets.sid'
CRAZY_COMETS_FT_BASE = 0x540F  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    subtune_indices: list[int] = [0, 1]
    if argv:
        subtune_indices = [int(x) for x in argv[0].split(',')]

    # Extract per subtune from CrazyComets (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=CRAZY_COMETS_SID,
            ft_base=CRAZY_COMETS_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def crazy_cometsV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for CrazyComets. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). CrazyComets doesn't do this; keeping them would
    #     overwrite CrazyComets's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def crazy_cometsV3 : USFSong := {{
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/crazy_comets/codegen/CrazyComets/SongData.lean'
    )
