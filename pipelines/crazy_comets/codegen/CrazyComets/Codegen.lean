/-
  CrazyComets.Codegen — Player codegen for Rob Hubbard's CrazyComets on the Run,
  ...
  CrazyComets-specific, are flagged inline:
...
import CrazyComets.SID
import CrazyComets.Asm6502
import CrazyComets.PSIDFile
import CrazyComets.USF
import CrazyComets.Constants

namespace CrazyCometsNS
...
/-- Voice-state init: zero olpos/wptr/pattlo/patthi for all three voices
    ...
    See docs/hubbard_crazy_comets_disassembly.s ($54FA/$54FB tick gate
    at $5054-$5061). -/
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in CrazyComets's V3 first-pattern
  ...
  -- (must disassemble CrazyComets's player to find what gets written to
...
--  not applicable in the cloned CrazyComets codegen. SidgenCrazyCometsMain.lean
--  provides its own main using `crazy_cometsV3` from CrazyCometsV3.lean.)
