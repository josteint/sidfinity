...
3. **Actual MoN SFX player binary** — the relocator expects a separate "player and data" file. HVSC SID files using the MoN engine contain this player inline. Disassembling one of those SIDs from `hvsc85/MUSICIANS/M/Maniacs_of_Noise/` would give the exact memory layout.
