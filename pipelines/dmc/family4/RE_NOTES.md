...
- **EARLY <64 = 239 (71%) = THE LEADIN (dominant next blocker).** The composer's
  ...
  shaped like the orig's — the earlier LEFT_SPDCTR=mem[base+$16] attempt FAILED
  precisely because the composer's reload-to-speed spdctr ≠ the orig toggle; it
  only represents phase 0/1). Secondary thread: crack why the trichotomy passes
  Plasmostyle but not 2_Hours (both $1016=1; discriminator = idle-note-0 →
  suspect the hard-restart SR=0 write on the extra idle play). dist of
  mem[base+$16] over partials: {0:111, 1:222, 2:2, 255:1}.
