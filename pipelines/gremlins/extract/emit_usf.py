"""Generate USF v3 Gremlins on the Run data as a Lean file.
...
Discovered freq_table_addr for Gremlins: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    Gremlins does NOT do this — its pitch 104 is a real freq lookup
...
def main(argv: list[str] | None = None) -> None:
    ...
    subtune_indices: list[int] = [0, 1, 2, 3, 4, 5, 6]
    if argv:
        subtune_indices = [int(x) for x in argv[0].split(',')]

    # Extract per subtune from Gremlins (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=GREMLINS_SID,
            ft_base=GREMLINS_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 Gremlins on the Run data",
    ...
    out.append(
        f"def gremlinsV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for Gremlins. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). Gremlins doesn't do this; keeping them would
    #     overwrite Gremlins's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def gremlinsV3 : USFSong := {{
  freqTable := gremlinsV3FreqTable
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/gremlins/codegen/Gremlins/SongData.lean'
    )
