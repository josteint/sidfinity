# Gremlins on the Run pipeline

End-to-end rebuild of Rob Hubbard's *Gremlins on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for Gremlins's engine quirks
...
explanation. The Gremlins-specific differences are inside the codegen:

| File | Gremlins-only addition |
|---|---|
| `codegen/Gremlins/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/Gremlins/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
python -m pipelines.gremlins.extract.emit_usf 0,1,2       # all three music tracks
```
...
invariant safe while Gremlins was being developed. The two pipelines can
