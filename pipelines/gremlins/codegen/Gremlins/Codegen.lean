/-
  Gremlins.Codegen — Player codegen for Rob Hubbard's Gremlins on the Run,
  ...
  Gremlins-specific, are flagged inline:
...
import Gremlins.SID
import Gremlins.Asm6502
import Gremlins.PSIDFile
import Gremlins.USF
import Gremlins.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig Gremlins: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for Gremlins. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in Gremlins's V3 first-pattern
  ...
  -- (must disassemble Gremlins's player to find what gets written to
...
--  not applicable in the cloned Gremlins codegen. SidgenGremlinsMain.lean
--  provides its own main using `gremlinsV3` from GremlinsV3.lean.)
