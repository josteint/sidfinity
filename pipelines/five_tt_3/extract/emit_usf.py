"""Generate USF v3 FiveTt3 on the Run data as a Lean file.
...
Discovered freq_table_addr for FiveTt3: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    FiveTt3 does NOT do this — its pitch 104 is a real freq lookup
...
FIVE_TT_3_SID = '/home/jtr/sidfinity/pipelines/five_title_tunes/work_subs/sub_3.sid'
FIVE_TT_3_FT_BASE = 0x2BA0  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from FiveTt3 (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=FIVE_TT_3_SID,
            ft_base=FIVE_TT_3_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 FiveTt3 on the Run data",
    ...
    out.append(
        f"def five_tt_3V3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for FiveTt3. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). FiveTt3 doesn't do this; keeping them would
    #     overwrite FiveTt3's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def five_tt_3V3 : USFSong := {{
  freqTable := five_tt_3V3FreqTable
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/five_tt_3/codegen/FiveTt3/SongData.lean'
    )
