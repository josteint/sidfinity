/-
  FiveTt3.Codegen — Player codegen for Rob Hubbard's FiveTt3 on the Run,
  ...
  FiveTt3-specific, are flagged inline:
...
import FiveTt3.SID
import FiveTt3.Asm6502
import FiveTt3.PSIDFile
import FiveTt3.USF
import FiveTt3.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig FiveTt3: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for FiveTt3. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in FiveTt3's V3 first-pattern
  ...
  -- (must disassemble FiveTt3's player to find what gets written to
...
def generateSID (song : USFSong) (baseAddr : UInt16 := 0x1000) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the FiveTt3 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the FiveTt3 binary at $84E5..$84E7
...
--  not applicable in the cloned FiveTt3 codegen. SidgenFiveTt3Main.lean
--  provides its own main using `five_tt_3V3` from FiveTt3V3.lean.)
