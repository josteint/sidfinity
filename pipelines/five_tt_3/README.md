# FiveTt3 on the Run pipeline

End-to-end rebuild of Rob Hubbard's *FiveTt3 on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for FiveTt3's engine quirks
...
explanation. The FiveTt3-specific differences are inside the codegen:

| File | FiveTt3-only addition |
|---|---|
| `codegen/FiveTt3/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/FiveTt3/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.five_tt_3.extract.emit_usf            # subtune 0 only
python -m pipelines.five_tt_3.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_five_tt_3
./.lake/build/bin/sidgen_five_tt_3
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/sub_3.sid \
    five_tt_3.sid
...
invariant safe while FiveTt3 was being developed. The two pipelines can
