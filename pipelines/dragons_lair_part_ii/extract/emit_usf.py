"""Generate USF v3 DragonsLairPartIi on the Run data as a Lean file.
...
Discovered freq_table_addr for DragonsLairPartIi: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    DragonsLairPartIi does NOT do this — its pitch 104 is a real freq lookup
...
DRAGONS_LAIR_PART_II_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Dragons_Lair_Part_II.sid'
DRAGONS_LAIR_PART_II_FT_BASE = 0xC402  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from DragonsLairPartIi (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=DRAGONS_LAIR_PART_II_SID,
            ft_base=DRAGONS_LAIR_PART_II_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 DragonsLairPartIi on the Run data",
    ...
    out.append(
        f"def dragons_lair_part_iiV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for DragonsLairPartIi. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). DragonsLairPartIi doesn't do this; keeping them would
    #     overwrite DragonsLairPartIi's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def dragons_lair_part_iiV3 : USFSong := {{
  freqTable := dragons_lair_part_iiV3FreqTable
  ...
  playRate := .vbi
  engineQuirks := {quirks}
  title := "DragonsLairPartIi on the Run"
  author := "Rob Hubbard"
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/dragons_lair_part_ii/codegen/DragonsLairPartIi/SongData.lean'
    )
