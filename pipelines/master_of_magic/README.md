# MasterOfMagic on the Run pipeline

End-to-end rebuild of Rob Hubbard's *MasterOfMagic on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for MasterOfMagic's engine quirks
...
explanation. The MasterOfMagic-specific differences are inside the codegen:

| File | MasterOfMagic-only addition |
|---|---|
| `codegen/MasterOfMagic/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/MasterOfMagic/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.master_of_magic.extract.emit_usf            # subtune 0 only
python -m pipelines.master_of_magic.extract.emit_usf 0,1,2       # all three music tracks
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/Master_of_Magic.sid \
    master_of_magic.sid
...
invariant safe while MasterOfMagic was being developed. The two pipelines can
