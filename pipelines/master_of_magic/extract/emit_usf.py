"""Generate USF v3 MasterOfMagic on the Run data as a Lean file.
...
Discovered freq_table_addr for MasterOfMagic: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    MasterOfMagic does NOT do this — its pitch 104 is a real freq lookup
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from MasterOfMagic (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=MASTER_OF_MAGIC_SID,
            ft_base=MASTER_OF_MAGIC_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def master_of_magicV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for MasterOfMagic. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). MasterOfMagic doesn't do this; keeping them would
    #     overwrite MasterOfMagic's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def master_of_magicV3 : USFSong := {{
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/master_of_magic/codegen/MasterOfMagic/SongData.lean'
    )
