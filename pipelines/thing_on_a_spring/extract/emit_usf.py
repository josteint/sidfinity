"""Generate USF v3 ThingOnASpring on the Run data as a Lean file.
...
Discovered freq_table_addr for ThingOnASpring: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    ThingOnASpring does NOT do this — its pitch 104 is a real freq lookup
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from ThingOnASpring (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=THING_ON_A_SPRING_SID,
            ft_base=THING_ON_A_SPRING_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 ThingOnASpring on the Run data",
        f"-- Subtunes: {subtune_indices} (0-indexed; PSID subtunes "
        f"{[s + 1 for s in subtune_indices]})",
        "import ThingOnASpring.USF",
        "",
    ]

    out.append(
        f"def thing_on_a_springV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for ThingOnASpring. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). ThingOnASpring doesn't do this; keeping them would
    #     overwrite ThingOnASpring's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def thing_on_a_springV3 : USFSong := {{
  freqTable := thing_on_a_springV3FreqTable
  ...
  playRate := .vbi
  engineQuirks := {quirks}
  title := "ThingOnASpring on the Run"
  author := "Rob Hubbard"
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/thing_on_a_spring/codegen/ThingOnASpring/SongData.lean'
    )
