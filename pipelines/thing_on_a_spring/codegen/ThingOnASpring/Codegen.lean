/-
  ThingOnASpring.Codegen — Player codegen for Rob Hubbard's ThingOnASpring on the Run,
  ...
  ThingOnASpring-specific, are flagged inline:
...
import ThingOnASpring.SID
import ThingOnASpring.Asm6502
import ThingOnASpring.PSIDFile
import ThingOnASpring.USF
import ThingOnASpring.Constants

namespace ThingOnASpringNS
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig ThingOnASpring: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for ThingOnASpring. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in ThingOnASpring's V3 first-pattern
  ...
  -- (must disassemble ThingOnASpring's player to find what gets written to
...
--  not applicable in the cloned ThingOnASpring codegen. SidgenThingOnASpringMain.lean
--  provides its own main using `thing_on_a_springV3` from ThingOnASpringV3.lean.)

end ThingOnASpringNS
