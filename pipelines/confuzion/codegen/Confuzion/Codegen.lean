...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  let encodeNote (n : USFNoteEvent) (tempo : Nat) : List UInt8 :=
    ...
    let instBits : UInt8 := n.instrument.toUInt8
    let hasNewInst : Bool := (instBits &&& 0x80) = 0
    let cmd : UInt8 :=
      durByte |||
      (if (instBits &&& 0x20) ≠ 0 then 0x20 else 0) |||
      (if isTie || (instBits &&& 0x40) ≠ 0 then 0x40 else 0) |||
      (if hasNewInst then 0x80 else 0)
    let instByte : List UInt8 := if hasNewInst then [instBits &&& 0x1F] else []
    let pitchByte : List UInt8 :=
      if isTie then [] else
        match n.kind with
