"""Generate USF v3 Confuzion on the Run data as a Lean file.
...
Discovered freq_table_addr for Confuzion: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    Confuzion does NOT do this — its pitch 104 is a real freq lookup
...
CONFUZION_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Confuzion.sid'
CONFUZION_FT_BASE = 0x0AFD  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from Confuzion (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=CONFUZION_SID,
            ft_base=CONFUZION_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def confuzionV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for Confuzion. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). Confuzion doesn't do this; keeping them would
    #     overwrite Confuzion's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def confuzionV3 : USFSong := {{
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/confuzion/codegen/Confuzion/SongData.lean'
    )
