"""Generate USF v3 HumanRace on the Run data as a Lean file.
...
Discovered freq_table_addr for HumanRace: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    HumanRace does NOT do this — its pitch 104 is a real freq lookup
...
HUMAN_RACE_SID = '/home/jtr/sidfinity/data/C64Music/MUSICIANS/H/Hubbard_Rob/Human_Race.sid'
HUMAN_RACE_FT_BASE = 0x0CE4  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from HumanRace (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=HUMAN_RACE_SID,
            ft_base=HUMAN_RACE_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out.append(
        f"def human_raceV3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for HumanRace. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). HumanRace doesn't do this; keeping them would
    #     overwrite HumanRace's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def human_raceV3 : USFSong := {{
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/human_race/codegen/HumanRace/SongData.lean'
    )
