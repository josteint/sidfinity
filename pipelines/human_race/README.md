...
```bash
python -m pipelines.human_race.extract.emit_usf            # subtune 0 only
python -m pipelines.human_race.extract.emit_usf 0,1,2       # all three music tracks
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/Human_Race.sid \
    human_race.sid
