/-
  HumanRace.Codegen — Player codegen for Rob Hubbard's HumanRace on the Run,
  ...
  HumanRace-specific, are flagged inline:
...
import HumanRace.SID
import HumanRace.Asm6502
import HumanRace.PSIDFile
import HumanRace.USF
import HumanRace.Constants

namespace HumanRaceNS
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in HumanRace's V3 first-pattern
  ...
  -- (must disassemble HumanRace's player to find what gets written to
...
def generateSID (song : USFSong) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the HumanRace binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the HumanRace binary at $84E5..$84E7
...
--  not applicable in the cloned HumanRace codegen. SidgenHumanRaceMain.lean
--  provides its own main using `human_raceV3` from HumanRaceV3.lean.)
