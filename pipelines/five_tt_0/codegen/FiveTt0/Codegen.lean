/-
  FiveTt0.Codegen — Player codegen for Rob Hubbard's FiveTt0 on the Run,
  ...
  FiveTt0-specific, are flagged inline:
...
import FiveTt0.SID
import FiveTt0.Asm6502
import FiveTt0.PSIDFile
import FiveTt0.USF
import FiveTt0.Constants
...
def emitExecVoice (cb : CodeBuilder) (song : USFSong) : CodeBuilder := Id.run do
  ...
  -- Orig FiveTt0: HR fires 2 frames before note-load (writelog trace), so the
  -- threshold here is v_dur == 1 for FiveTt0. Commando used cmp_imm 2; the
...
def emitNoteLoadPath (cb : CodeBuilder) (song : USFSong) : CodeBuilder :=
  ...
  -- second note-load (at frame 65 in FiveTt0's V3 first-pattern
  ...
  -- (must disassemble FiveTt0's player to find what gets written to
...
def generateSID (song : USFSong) (baseAddr : UInt16 := 0x1000) (debug : Bool := false) : Bytes := Id.run do
  ...
  cb := cb.label "v_pwhi"
  cb := cb.emitData [0, 0, 0]
  -- pulsedir init values extracted from the FiveTt0 binary at $84E8..$84EA
  ...
  -- pulsedelay init values extracted from the FiveTt0 binary at $84E5..$84E7
...
--  not applicable in the cloned FiveTt0 codegen. SidgenFiveTt0Main.lean
--  provides its own main using `five_tt_0V3` from FiveTt0V3.lean.)
