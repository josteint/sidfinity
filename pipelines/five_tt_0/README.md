# FiveTt0 on the Run pipeline

End-to-end rebuild of Rob Hubbard's *FiveTt0 on the Run* (1985) SID. Same shape
as the Commando pipeline; cloned and extended for FiveTt0's engine quirks
...
explanation. The FiveTt0-specific differences are inside the codegen:

| File | FiveTt0-only addition |
|---|---|
| `codegen/FiveTt0/USF.lean` | `skydive : Bool` field on `USFInstrument` |
| `codegen/FiveTt0/Codegen.lean` | Skydive emit block; v_pitch alias-store into freq table; PWM init data extracted from binary; HR threshold = 1 |
...
```bash
python -m pipelines.five_tt_0.extract.emit_usf            # subtune 0 only
python -m pipelines.five_tt_0.extract.emit_usf 0,1,2       # all three music tracks
```

Build and run:

```bash
lake build sidgen_five_tt_0
./.lake/build/bin/sidgen_five_tt_0
```
...
python src/writelog_grade.py \
    data/C64Music/MUSICIANS/H/Hubbard_Rob/sub_0.sid \
    five_tt_0.sid
...
invariant safe while FiveTt0 was being developed. The two pipelines can
