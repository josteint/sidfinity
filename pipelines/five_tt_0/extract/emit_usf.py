"""Generate USF v3 FiveTt0 on the Run data as a Lean file.
...
Discovered freq_table_addr for FiveTt0: $8400 (via src/sidxray/discover.py).
"""
...
def gen_freq_table(T: list[int]) -> str:
    ...
    FiveTt0 does NOT do this — its pitch 104 is a real freq lookup
...
FIVE_TT_0_SID = '/home/jtr/sidfinity/pipelines/five_title_tunes/work_subs/sub_0.sid'
FIVE_TT_0_FT_BASE = 0x0F6A  # discovered via src/sidxray/discover.py
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Extract per subtune from FiveTt0 (not Commando). Discovery-derived
    ...
    extracts: list[ExtractedSong] = [
        extract(
            subtune=s,
            sid_path=FIVE_TT_0_SID,
            ft_base=FIVE_TT_0_FT_BASE,
            default_pw_min=0x08,
            default_pw_max=0x0E,
        )
    ...
    out = [
        "-- Auto-generated USF v3 FiveTt0 on the Run data",
    ...
    out.append(
        f"def five_tt_0V3FreqTable : USFFreqTable := "
    ...
    # Engine quirks for FiveTt0. Same Hubbard engine as Commando, so:
    ...
    #     per-voice state). FiveTt0 doesn't do this; keeping them would
    #     overwrite FiveTt0's real freq value at slot 104 ($4141) with
    ...
    out.append(f"""def five_tt_0V3 : USFSong := {{
  freqTable := five_tt_0V3FreqTable
  ...
  released := "1985 Gremlin Graphics"
    ...
    out_path = os.path.join(
        repo_root, 'pipelines/five_tt_0/codegen/FiveTt0/SongData.lean'
    )
