...
def decode_instrument(inst: int, binary: bytes, load_addr: int) -> InstrumentModel:
    """Decode instrument `inst` from its 8-byte table row."""
    off = INSTR_TABLE_BASE + inst * INSTR_SIZE - load_addr
    ...
    arpeggio = ArpSpec() if (fx & 0x04) else None
...
def decode_all(sid_path: str) -> list[InstrumentModel]:
    from src.hubbard_emu import load_sid
    _, binary, load_addr = load_sid(sid_path)
    return [decode_instrument(i, binary, load_addr)
