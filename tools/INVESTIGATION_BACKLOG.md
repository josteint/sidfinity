...
| `pipelines/future_composer/pattern_stream_decode.py` | Decode FC pattern stream bytes as readable command list ($Cx wave/inst, $Fx markers, glide triples, etc.) Both pattern and seq streams. | (in this file) |
| `pipelines/future_composer/state_map_gen.py` + per-engine `state_map.py` annotations | Auto-derive state_diff map files from xa65 labels joined with per-engine state-address annotations. `--engine ENGINE --voice {1,2,3,all}`. | (in this file) |
...
| `siddump --reinit-snapshot PC LO-HI` | GROUND-TRUTH PC-triggered RAM-window capture (Phase 1 of the native-capture plan): COLD = RAM[LO..HI] at the first play-vector entry (post-init); WARM = at the first play-vector entry after PC EXECUTES (= end of that play()). One `SNAP:COLD=<hex>\|WARM=<hex>` line. Execution vs data-read discriminated by the ≥3-consecutive-ascending-reads bus signature (ledger C36 — a bare addr==PC check false-fires on data reads). Consumer: DMC `_reinit_windows_via_siddump` (the C19 shape-B ghost capture; replaced + deleted the py65 path). | ledger [C36](../docs/ledger/C36.md) + CLAUDE.md |
| `tools/divergence_triage.py ORIG REBUILD [--engine E]` | ENGINE-BLIND divergence classifier (prototype, 2026-07-25): localizes the first writelog divergence (Trap-C-free PER-IRQ capture — robust to the CIA "diverge at position 0" artifact that `find_first_divergence`'s naive per-frame compare hits), then runs writelog-SIGNATURE detectors that suggest the likely ledger class with evidence — collapsing the manual "which C-entry is this?" triage. Blind detectors (work on ANY family): length-tail→C24/C9/C25/C12, cross-chip→C28, wrong-value-freq→C6/C22, reorder/missing→C16, global-reg→C10; distinguishes same-reg WRONG-VALUE from diff-reg REORDER. Engine-PARAMETRIZED detectors plug in via the `ENGINE_DETECTORS` registry (the `divergence_census`/`select_regression_portfolio` pattern: blind core + one entry per family; `dmc` routes freq divergences to `dmc_offtable_probe`). Validated on Hexzakk (→C6, matches the 177 verify localization) + Mothafucka_2SID (→reorder) + synthetic length/cross-chip. Prototype: registry has only the dmc off-table adapter; more per-engine probes + more blind signatures (C22-arming, C34) are v2. | (this file + CLAUDE.md) |
| `siddump --pc-watch LIST BEFORE-AFTER [--pc-watch-first] [--pc-watch-abs LO-HI]` | GROUND-TRUTH executed-PC event stream (native-capture Phase 2): one `\|PW:<pc>:<a>:<x>:<y>:<playidx>:<relwin>:<abswin>` event per EXECUTION of a watched PC (exact hex, or `*XX` low-byte pattern), C36-discriminated, with A/X/Y (overlay MOS6510 getters), the play-invocation index (0 = init) and RAM windows captured AT the hit. Serves landing/reachability observations: DMC `_observe_dispatch` (C31 compilation dispatch — replaced + deleted the py65 loop; `*00,*48` landing low-bytes, predicates replayed over the events); designed to also serve the C18 play-phase observers (exact PCs + X + playIdx). Registers sample at the PC+2 read — pre-instruction for ≥3-byte sites (JMP/`LDA abs,X`), post for 2-byte ones. | ledger [C36](../docs/ledger/C36.md) + CLAUDE.md |
...
| `pipelines/future_composer/pattern_stream_verify.py` | USF roundtrip check for the pattern-stream region. Verifies orig vs rebuild bytes match, accounting for `featuredriven_addr_shift` and pointer fixup. Catches data-emission regressions early. | (in this file + CLAUDE.md) |
...
| `pipelines/dmc/canon_diff.py [--status JSONL] [--csv]` | A-PRIORI WEDGE ENUMERATOR for a canon-player family: linear-align each member's reachable player code to the canonical player binary + diff OPCODES and in-player OPERAND-REPOINTS, Δ-mode-filter bulk state/table relocations, cluster by canon site, tag handled/NEW, and split each cluster into partial/full carriers (`--status`). The proactive complement to the reactive `_*_probe` detectors + a completeness audit of their true carrier counts. Proved DMC family-1's wedge space is ~fully handled: of 188 partials, 78% carry NO code wedge (off-table/CIA residue), 17% a handled wedge, only 9 (4%) a genuine unhandled patch — all singletons. LIMIT: misses immediate-value tweaks + re-assembled members (linear-align only). | [reference_dmc_canon_diff](.claude/memory/reference_dmc_canon_diff.md) + CLAUDE.md |

| `tools/write_timing_delta.py` + `tools/write_timing_sweep.py` | **HOW FAR a write moved INSIDE its play() burst** — the quantity the core tenet's Trap B declares inaudible, never measured until 2026-09-01. Regroups both write logs into true play() invocations with ABSOLUTE cycles (from `--per-irq-debug`'s stderr, so siddump stays untouched) and reports ONSET + SPREAD + burst WIDTH. ⚠ Must RECOVER the play-index offset (C21 — assuming 0 fails FLATTERINGLY), work PER CHIP (C28), and report a self-driven RSID as NOT MEASURABLE. The sweep's 15 s window RANKS; re-measure the top at full songlength. | backlog 39 |
| `tools/render_pair.py` | Render orig + rebuild to WAV for a targeted listening test. Docstring carries the warning: no sample-level null test, no hand-rolled spectral norm — both were tried and withdrawn. | backlog 39 |
| `tools/register_ownership.py` | **Which CODE writes which SID register**, so a music+digi member can get the split verdict. pc-traces the busiest window; ownership = the digi register's writer PCs DISJOINT from every other register's. ⚠ Classify by PC LOCALITY not write RATE (rate is a property of a LOOP, ownership of an ENGINE — a rate threshold reads an unrolled digi burst as "music"); ⚠ scan the ratified songlength, not a fixed N (a 25 s scan reported six members as having no digi; one starts its digi 430 s in). | [feedback_physical_constant_over_threshold](.claude/memory/feedback_physical_constant_over_threshold.md) |
| `tools/verdict_gaps.py` | Every non-DMC family's "member with no row / row with no member" check, asking each batch's own `members()`. Reports FULL-over-CLAIMED beside FULL-in-store — the gap is the dead rows an HVSC update leaves behind. Found fc_standard 2604→2572 and music_assembler 4021→4002. | backlog 37 |

## Backlog (not built yet)

**Standing default (2026-07-24): observe from the ground-truth engine
(libsidplayfp), not py65.** py65 is a reimplementation that DIVERGES on
uninitialized/environment reads (cost a session on DMC Roots — see
`feedback_ground_truth.md`); libsidplayfp is ground truth by construction.
**Architecture DECIDED 2026-07-25** (Phase-0 design-space survey →
...
in-process pybind11/nanobind binding over the SAME taps is a deferred
escalation for genuinely un-declarative sites; VICE-monitor / DBI / record-
then-query rejected (ground truth / no JIT / wrong workload shape). Plan +
inventory: [`docs/siddump_native_capture_plan.md`](../docs/siddump_native_capture_plan.md).

| Idea | Use case | Rough ROI | Build estimate |
|---|---|---|---|
| **Native py65-replacement — Phase 2 remainder + Phase 3** (Phase 0+1 DONE; Phase 2 STARTED 2026-07-25: `--pc-watch` built, `_observe_dispatch` migrated + py65 loop deleted — spec-identical on all 5 observe-path members, declarative-CLI fit CONFIRMED, no binding needed so far) | Remaining Phase 2: `_observe_play_phases` (canon) DONE 2026-07-25 (204-carrier A/B, effective-schedule identical, py65 twin deleted; the anchor-at-first-event lesson — the play counter is a bus-read proxy, an init data-read of the play vector shifts every index — is in the observer docstring). ASSESSED + LEFT ON py65 (not migration targets): the DMC multi-SID trio `_observe_play_phases_chip`/`multisid_active_chips`/`_observe_player_bases` (2d — not divergence-prone/slow; the per-chip `--pc-watch` anchor breaks cross-chip phase alignment for complementary schedules, Cow_Anus_Fucked FULL→PARTIAL, reverted) and `_postinit_window` (2c — an IDEALIZED SIMULATION, not an observation: the real machine always carries psiddrv + environment state the pipeline's idealized view excludes; Super_Seven sub 1 went partial; boundary in the plan's Class B + the function docstring). DMC candidate DONE (Phase 2e 2026-07-25): `_observe_play_phases_writes` migrated — `_observe_play_phases_pctrace` (ground truth) promoted to primary + py65 observer deleted. A/B over all 129 f1 writes-with-P carriers: pctrace gives IDENTICAL schedules on every S-phase slow-tempo carrier, so the final gate keeps allowing S; only 3 effective changes, all already PARTIAL. NB NO speed win (for short ~12-16f observers, siddump subprocess overhead offsets native speed — the speed benefit is deep-playback-only); value = ground truth + one fewer py65 site. **Phase 3 DONE (audit 2026-07-25): the CORRECTNESS goal is MET — no divergence-driven migration remains.** All ~32 non-DMC py65 sites classified: every divergence-prone (C29) site is in DMC (handled); non-DMC sites are init-only clean-engine reads (correctly on py65), dev tools, or SIMULATIONS that must not migrate (the audit found + docstring-guarded the 2nd instance, MA `heterogeneous._landing_memory`, before anyone tried). Only 3 sites trip the rule on the **slow** axis alone (jay_derrett `_run_play_capture` ~2000f / `type_b` ~1000f / `inst_program.capture` ~1500f) — clean, low-volume, non-batched, zero-correctness-benefit → deferred (do only on a speed need). Tooling hygiene (separate from this initiative): `src/usf/audit.py` dup of `voice_writelog.py`, `hubbard_emu.verify_against_py65` (proxy-vs-proxy), `jay_derrett/build.capture_writes_via_py65` (test-only) — deletion/consolidation candidates. | Correctness goal MET; remainder is low-ROI speed. | 3 slow sites optional |
| ~~**`dmc_offtable_probe` divergence-proximity gate**~~ **DONE (r116, 2026-07-24).** | The probe now pc-traces only a ±3-frame window around the divergence (per-irq play index converted for CIA tunes) and reports every proximate candidate; a value found only outside the window is reported as "NOT an off-table read at the divergence" with the far matches labeled by-value coincidences. Validated: Fantastic_Dreams (3 near candidates incl. the wavepos class), Long_Time (correct bow-out where the old scan would have blamed idx 140). Historical mis-fires: 6 (wavepos 3×, Real_Hardcore idx 150, r111 Rock_Tec_Tec idx 150, Psycho_One $172A). ⚠ 7th (r141, Hexzakk): the proximity gate is BLIND to the freq-lo/hi CACHE vars (canon $173F-$1744) — the cache write is in the SAME chain as the diverging store, so it is proximate BY CONSTRUCTION and always holds the diverging value; a candidate whose source addr is the cache should be auto-labeled a self-echo, never a lead. Also: the member was +\$10-layout-shifted, so canon-addressed candidates named the wrong vars (the dmc_state_addr rule). | — | done |
...
| `pipelines/future_composer/pattern_stream_decode.py` | Given (SID, engine config, subtune, voice), decode pattern stream as human-readable command list | Saves 30-60 min/session on pattern-dispatch bugs | 1-2 hours, engine-specific |
...
| `pipelines/future_composer/pattern_stream_decode.py` | BUILT (commit 7bb6abe). |
| `tools/disasm_diff.py` | BUILT (commit 6db2726). |
| State map generator | BUILT as `pipelines/future_composer/state_map_gen.py` (commit 0bdc98e). |
...
| Pattern-stream USF-vs-binary verifier | BUILT as `pipelines/future_composer/pattern_stream_verify.py`. |
