...
`feedback_ground_truth.md`); libsidplayfp is ground truth by construction and
~100-1000× faster. The GOAL is settled; the ARCHITECTURE is NOT — the initiative
starts with a **research phase** (survey emulator-introspection / DBI prior art:
VICE binary monitor, QEMU plugins, DTrace/eBPF, record-and-query, native
bindings) before committing to a mechanism. Full plan + inventory + strawman +
phased path: [`docs/siddump_native_capture_plan.md`](../docs/siddump_native_capture_plan.md).

| Idea | Use case | Rough ROI | Build estimate |
|---|---|---|---|
| **Native py65-replacement — research FIRST, then build** (see the plan) | Serve the divergence-prone / slow py65 observations (deep-playback sims like the DMC ghost sim; init-landing / play-phase observers) from libsidplayfp instead of py65. Architecture undecided — Phase 0 is a design-space survey producing a decision doc; a strawman (siddump declarative flags + observe-only overlay taps like a PC-state hook / PC-attributed writelog) is in the plan as a baseline to compare against. | High — kills the py65-divergence bug class + big speedups; but scope depends on the Phase-0 decision. | Phase 0 research (deep-research skill) → then phased build |
