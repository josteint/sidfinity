...
|---|---|---|---|
| **`dmc_offtable_probe` divergence-proximity gate** | The probe's by-value attribution has now mis-fired 6× (C11 card: 3× wavepos; 2026-07-24 Real_Hardcore: reported "off-table idx 150 STATIC" for a first divergence at frame 6 whose $16DD read occurs only much LATER in the song — the real cause was a C18 phase-classification bug; r112 Rock_Tec_Tec: tool bowed out correctly but the r111 idx-150 hit was by-value; r116 Psycho_One: reported "idx 131 → $172A LIVE (sectpos)" when the diverging $2A was a glide-arrival idx-255 read whose value coincided with $172A's — found by pc-tracing the ACTUAL diverging write). Fix: require the matched off-table load to occur within ~2 play() invocations of the diverging write's position (the trace has invocation boundaries), else report "no off-table read near the divergence" and point at the divergence-recipe path. | High — each mis-fire costs a wrong-direction start on exactly the investigations the tool exists to shortcut. 6 mis-fires and counting. | ~30 min |
