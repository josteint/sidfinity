...
|---|---|---|---|
| **Native py65-replacement — Phases 2-3** (Phase 0 decision doc DONE 2026-07-25; Phase 1 DONE: `--reinit-snapshot` + the DMC ghost sim migrated, py65 path deleted) | Remaining: Phase 2 = init-landing / reachable-PC report flag (Class C: compilation dispatch `_observe_dispatch`, play-phase / base observers) — the decision checkpoint for whether any site is genuinely un-declarative (→ build the binding, scoped). Phase 3 = sweep remaining Class-D / divergence-prone py65 sites across families. Class A (`inst_program`) stays py65. Gates per site: byte-identical extract output + family verify + delete the py65 path after green. | High — kills the py65-divergence bug class site by site. | Phase 2: ~2-3 h (flag + 2 migrations + gates) |
