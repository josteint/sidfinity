...
|---|---|---|---|
| **Native py65-replacement — Phase 2 remainder + Phase 3** (Phase 0+1 DONE; Phase 2 STARTED 2026-07-25: `--pc-watch` built, `_observe_dispatch` migrated + py65 loop deleted — spec-identical on all 5 observe-path members, declarative-CLI fit CONFIRMED, no binding needed so far) | Remaining Phase 2: `_observe_play_phases` (canon) DONE 2026-07-25 (204-carrier A/B, effective-schedule identical, py65 twin deleted; the anchor-at-first-event lesson — the play counter is a bus-read proxy, an init data-read of the play vector shifts every index — is in the observer docstring). ASSESSED + LEFT ON py65 (not migration targets): the DMC multi-SID trio `_observe_play_phases_chip`/`multisid_active_chips`/`_observe_player_bases` (2d — not divergence-prone/slow; the per-chip `--pc-watch` anchor breaks cross-chip phase alignment for complementary schedules, Cow_Anus_Fucked FULL→PARTIAL, reverted) and `_postinit_window` (2c — an IDEALIZED SIMULATION, not an observation: the real machine always carries psiddrv + environment state the pipeline's idealized view excludes; Super_Seven sub 1 went partial; boundary in the plan's Class B + the function docstring). Remaining genuine candidate: `_observe_play_phases_writes` (re-assembled route; needs the WRITE footprint per play() which `--pc-watch` lacks, but its ground-truth twin `_observe_play_phases_pctrace` already exists → a fallback-order swap gated by a carrier census). Phase 3 = sweep remaining Class-D / divergence-prone py65 sites across families. Class A (`inst_program`) stays py65. Gates per site: identical extract output + family verify + delete the py65 path after green. | High — kills the py65-divergence bug class site by site. | play-phase migration: ~1-2 h |
