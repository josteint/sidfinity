...
def _run_memwatch(sid_path: str, subtune: int, duration: float,
                  addrs: list[int]) -> list[dict[int, int]]:
    """Run siddump --memwatch and parse the per-frame snapshots.
    Returns a list of dicts (one per frame) mapping addr → byte value.
    """
    ...
    for line in r.stdout.splitlines():
        if '|M:' not in line:
            ...
            continue
        snap = {}
        m_section = line.split('|M:', 1)[1]
        # In case writelog appended after, strip from |W
        if '|W' in m_section:
...
def state_diff(orig_path: str, rebuild_path: str,
    ...
    orig_frames = _run_memwatch(orig_path, subtune, duration, orig_addrs)
    rebuild_frames = _run_memwatch(rebuild_path, subtune, duration,
                                    rebuild_addrs)
