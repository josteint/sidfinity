 ...
 *
 * Modified by SIDfinity: public observe-only register accessors for the
 * c64cpubus PC-watch tap (getRegA/X/Y read CPU state, never change it).
 *
...
public:
    ...
    // SIDfinity: observe-only register accessors for the c64cpubus PC-watch
    // tap (siddump --pc-watch). Read-only; sampled between bus cycles.
    inline uint8_t getRegA() const { return Register_Accumulator; }
    inline uint8_t getRegX() const { return Register_X; }
    inline uint8_t getRegY() const { return Register_Y; }
