...
bool sidplayfp::enableWriteLog(unsigned int sidNum, bool enable)
{
    return sidplayer.enableWriteLog(sidNum, enable);
}

bool sidplayfp::clearWriteLog(unsigned int sidNum)
{
    return sidplayer.clearWriteLog(sidNum);
}

static const std::vector<SidWrite> emptyLog;

const std::vector<SidWrite>& sidplayfp::getWriteLog(unsigned int sidNum)
{
