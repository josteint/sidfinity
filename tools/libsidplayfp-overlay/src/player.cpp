...
bool Player::enableWriteLog(unsigned int sidNum, bool enable)
{
    if (sidNum >= m_chips.size())
        return false;
    m_chips[sidNum]->enableWriteLog(enable);
    return true;
}

bool Player::clearWriteLog(unsigned int sidNum)
{
    if (sidNum >= m_chips.size())
        return false;
    m_chips[sidNum]->clearWriteLog();
    return true;
}

const std::vector<libsidplayfp::SidWrite>* Player::getWriteLog(unsigned int sidNum)
{
    if (sidNum >= m_chips.size())
        return nullptr;
