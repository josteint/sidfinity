...
def _build_state_map(cfg, state_mod, voices: list[int]) -> dict:
    """Build the {orig_addr: (rebuild_addr, label)} dict."""
    ...
    for ann_name, orig_base in state_mod.PER_VOICE_STATE.items():
        ...
        for v in voices:
            ...
            out[orig_base + offset] = (rebuild_base + offset, label)
    ...
    for ann_name, orig_addr in state_mod.SCALAR_STATE.items():
        ...
        if rebuild_addr is None:
            print(f'WARN: composer label not found: {composer_name} '
                  f'(annotated as {ann_name})', file=sys.stderr)
            continue
        out[orig_addr] = (rebuild_addr, ann_name)
...
def main() -> int:
    ...
    m = _build_state_map(cfg, state_mod, voices)
