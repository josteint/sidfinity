...
- **FIRST ASK WHOSE CAP IT IS (2026-07-22, Heavy_Metal_Deluxe_beta, DMC f1
  ...
  Doesn't help past the real bound: a 39-instrument sibling (Lane_Crazy) needs
  per-song instrument WINDOWS (each subtune runs one packed player and uses
  ≤11), which is the merge-side analogue of the emit-and-walk tier above.
