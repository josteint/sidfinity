...
### Third occurrence, same session — the head offsets chose the REFERENCE (2026-08-28)

The section above is a member wearing a head that is not its player's. The
mirror is a member whose head sends the DISPATCHER to the wrong reference, and
DMC v5 had 43 of them sitting in the unrouted bucket.

`_detect_v5` recognises the family-4 branch only when the jump table reads init
`base+$40` AND play `base+$95`. A family-4 PLAYER carrying any other head
therefore never reaches that branch: base is derived as `play−$A1` and the body
is compared against the family-3/5 reference, which fails at the play entry
(`$10A1`) — where the two players genuinely differ. The refusal is honest and
completely uninformative.

Measured: of the 128 unrouted members diverging at `$10A1`, **43 have the
family-4 play head verbatim** (`A5 FA 48 A5 FB 48 CE 16 10 30 1E …`), and 41 of
them are **median 96.0% byte-identical** to the family-4 reference over its
body — inside the genuine family-4 band (47-100%, versus 2.0% for the impostor
in the section above). They are family-4 players.

**RULE, stated for any N-reference dispatcher:** the head selects a CANDIDATE,
never THE reference. When the chosen reference's body compare fails, try the
others before refusing — a family with two known players has two references and
the compare is what distinguishes them. A refusal is only meaningful once every
reference has been offered.

⚠ AND CHECK THE OBVIOUS EXPLANATION BEFORE BELIEVING IT. Those same 43 members
also differ in which ZERO-PAGE POINTER the player uses (`$FA`/`$FB` vs canon
`$F8`/`$F9`), which reads as a masked-compare bug — zp is runtime state, like
the `$1006-$1040` work block the compare already ignores, and the composer
picks its own zp anyway. Re-running the whole body compare with every zp
operand treated as don't-care matched only **3 of 50**: the other 43 still
differ at `$10A7`, because the zp difference is a SYMPTOM of being a different
player, not the cause of the refusal. Fixing the "obvious" thing would have
changed nothing and hidden the real finding.
