...
- **Consumers:** DMC v4 filter-def walk (2026-07-02, +17 FULL): repeat>5 reloads
  ...
  nominal length. **Seen 6×+ now (freq C6, pulse, wave×3, filter) — canonicalize.**
  **DMC v4 IN-TABLE RUNAWAY (2026-07-29, r129):** the sibling of the off-table
  start — an IN-table wave program with NO marker before the table's nominal end.
  `_slice_wave`'s runaway branch was the LAST nominal-length bound left: it
  capped at table end and HELD the last step, but the engine never holds — the
  8-bit position INCs straight into the adjacent bytes (the packer truncates the
  ctrl/note tables to the used cells, so the walk crosses into the NOTE table's
  head — the two tables are `n_wave` apart — and the note reads run past the note
  table's end). TELL: a drum/wave voice repeats a too-short cycle where the orig
  plays extra garbage-looking steps then retriggers. Fix: route the runaway branch
  to `_resolve_wave_chain` (same simulator as off-table starts / marker
  underflows); the stated wave_table norm form absorbs the settled walk's extra
  cells (only the note-duration-bounded prefix is ever observable). Regression
  shape: a cap-held FULL member can only be FULL if the tail past the end is
  never played, so the extension adds only unplayed steps — verdict-neutral by
  construction; census the mechanical risks (norm-form fallback, shape caps).
  Occurrences: [[project_dmc]] r129.
