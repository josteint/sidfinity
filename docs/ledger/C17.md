...
- **Refinement (2026-08-11, Basic_Program NF — the SPREAD-CHORD completion):
  when the normal form's row-derivation seems to hit its limit, check whether
  the missing information is ALREADY in the USF as the decl vocabulary — THE
  DECLS ARE A GRAMMAR AND THE READER IS ITS PARSER.** A slow interpreter
  (BASIC) spreads one musical chord's writes over 3-4 frames, splitting
  MID-VOICE (v2's envelope beside v3's freq, then v3's envelope beside v1's
  gate); no (onset, voice) partition of tracker rows can re-derive that step
  grouping, which looks like a representation gap forcing the C17 template
  form. It is not: the writer's rows carry every part (tie-notes, timbre
  rests) and the stored per-step decls record exactly which voice-subsets
  formed steps. Reconstruction = parsing the per-(onset, voice) ordered event
  queues against that grammar: each step consumes the next unconsumed event
  of some voice subset (per-voice order preserved), must resolve a stored
  decl, and must have instruments established for its instrument registers.
  ⚠ The parse needs WHOLE-SONG BACKTRACKING (iterative, node-capped): a
  wrong local grouping changes the step COUNT, drifting every later step's
  global-event alignment, so dead ends surface hundreds of steps downstream.
  Candidate order must preserve the pre-parser precedence (combined first)
  so previously-parsing members take byte-identical paths. Landed +22 NF
  converts (Basic_Program adoption 304/493); stored corpus proven untouched
  (279/282 production verify, 3 pre-existing window artifacts). Occurrences
  in [[project_basic_program]].
