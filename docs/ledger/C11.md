...
- **Refinement (2026-08-26, v5 pulsepos/filterpos — the paged-pool refusal
  narrowed by MEASURING READ OCCURRENCE): a refusal gate keyed on ENUMERATED
  reach over-fires — measure whether the read HAPPENS before deciding it is
  live.** The v5 paged-pool work (C8 sixth widening) refused 4 members whose
  off-table freq reads land on the live pulsepos/filterpos block ($1800-$1803
  family-4). The reach model (`_assign_offtable_freq`: instrument x played
  notes x ALL transposes x offsets) enumerates far more reads than occur, and
  "lands on a live address" says nothing about what the read observes. The
  read-moment census (`measure_live_window_reads`: `siddump --pc-watch` on
  ALL 8 family-4 freq-table read sites — ws/ni/note-fetch/vib/glide-cmp, the
  all-read-sites rule — with `--pc-watch-abs` over the live window, per
  subtune, full verify window) split the 4 three ways:
  * Hardvibes_tune_1 — NO enumerated live-window read EVER fires: the
    records are inert, any static byte serves. BUILDS.
  * Psychodelic_Killer — idx 231 fires 8x, observes $0D EVERY time (while
    $1800 cycles 3+ values globally): the event-driven doctrine's
    stable-at-the-read case; serve the measured value. BUILDS.
  * Psychomusicdelirkill + Valtavirtaa — idx 136 observes 3 resp. 2 distinct
    values: genuinely live, no static byte serves; REFUSED (reason now names
    the inconstant key). These are the true option-(a) remainder.
  This began as a REFUSAL LADDER (never-read -> inert / constant ->
  measured override / inconstant -> refuse) on the native pc-watch capture
  (per the siddump native-capture decision; C36 execution-discriminated).
  **SUPERSEDED SAME DAY by the LIVE-POSITION FORM (owner-directed +
  approved 2026-08-26): v5's first live-signal serve.** The owner pointed
  out the ladder still baked coincidences (Psychodelic_Killer's constant
  $0D IS the live V1 pulsepos); the honest representation names the
  variable. Since `pulse_position`/`filter_position` are POSITION-VALUED
  (the wave_position problem), a plain redirect can't serve them under
  de-fused/paged pools — but the pulse/filter walk is LINEAR (no marker
  chains through foreign data, unlike wave), so
      orig cursor = own cursor + (orig start − own start)
  with the delta CONSTANT per instrument. USF carries the OFSIG names
  `pulse_position(vN)`/`filter_position()` in the record slots + one
  per-instrument byte `pulse_table_pos`/`filter_table_pos` (the
  wave_table_pos/record_offset §8-arrangement siblings) — no stated
  table, no layout preservation, paging intact. Composer: `iopd` stride-8
  delta table -> per-voice `pposd,x`/global `fposd` at note-init (beside
  the cursor restarts), serve subs `otv_flo/fhi` + cmp wrappers at EVERY
  freq-table read site (the all-read-sites rule; carry preserved across
  the sub — the family-4 wave-step chains its carry into `adc frqbias`).
  VALIDITY GATE (measured, kept from the ladder): every observed read
  value must lie inside a captured program region — past _PHASE_CAP
  truncation our cursor parks while the orig walks on (Valtavirtaa's
  start-2 program reaches pos 255; no read ever fires out there). All 4
  carriers pass → the refusal class is EMPTY; all 4 build as ordinary
  partials (Psychodelic_Killer's divergence byte-identical to the
  measured-constant build = the serve proven equivalent; the other
  divergences are unrelated family-4 residue). Gated to the
  overflow+live class = the 10 static-capture FULL carriers untouched;
  60/60 byte gate; corpus 12,595 re-parses after the OFSIG growth.
