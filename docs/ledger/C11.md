...
- **Refinement (2026-08-26, v5 pulsepos/filterpos — the paged-pool refusal
  ...
  * Psychomusicdelirkill + Valtavirtaa — idx 136 observes 3 resp. 2 distinct
  ...
  This is `_offtable_eventdriven`'s philosophy as a REFUSAL LADDER
  (never-read -> inert / constant -> measured override / inconstant ->
  refuse), implemented on the native pc-watch capture (per the siddump
  native-capture decision; C36 execution-discriminated) instead of a
  $D416-triggered snapshot — v5 still has NO redirect machinery, so the
  inconstant case stays refused rather than redirect-served. GATE: the
  measurement runs only for would-be-refused members (live flag + pool
  overflow) = zero-regression by construction; 60/60 byte gate.
