...
- **22nd occurrence (round 117, 2026-07-24) — $FF track-loop handler
  ...
  **naively detecting+handling shape B REGRESSES 12/14**: most carriers' $FF
  wrap falls PAST their verify window, so the ordinary loop-target build
  never reaches the re-init and already verifies FULL — forcing re-init
  handling on them breaks the majority. Shape-B handling MUST be gated on
  "the wrap is reached within songlength×1.1". The 2 genuinely-in-window
  shape-B members (For_Party_V_95, Roots) ALSO need the GHOST-UNIT tail
  (wrap voice ≠ last → init's RTS lets the play body run the remaining
  voices as ghost units with X past $18: a member-constant SID burst +
  state pokes) AND an unsolved V3-PHASE model (the ghost run preempts a
  voice's frame update, shifting its restart phase — not a state-block
  value). Reverted; full diagnosis in [[project_dmc]].
