...
- **22nd occurrence (round 117, 2026-07-24) — $FF track-loop handler
  ...
  **SHAPE B + the ghost tail (2026-07-24, investigated, DEFERRED — see
  [[project_dmc]]):** a second wedge shape `A9 00 / 4C <init>` (JMP straight
  to init, no neutered JSR) exists — 12 more f1 carriers (Hank/*). But
  **naively detecting+handling shape B REGRESSES 12/14** — the amend trap:
  the opcode-shape probe is UN-GATED w.r.t. whether the wrap is reached.
  DISCRIMINATOR (amend Step 3.1, measured on both sets): **does the orig
  re-init IN-WINDOW** (in-window $D418+ascending-clear burst)? ONLY
  For_Party passes; the other 11 shape-B carriers are PAST-WINDOW, so their
  $FF handler never executes in-window and a gated fix leaves them
  byte-identical FULL by construction (the regression was the un-gated
  detection + a `sectpos` compile bug, NOT a conflict). For_Party ALSO
  needs the GHOST-UNIT tail (wrap voice ≠ last → the play body runs the
  remaining voices as ghost units with X past $18: a member-constant SID
  burst + state pokes) AND an unsolved V3-PHASE model (the ghost run
  preempts a voice's frame update, shifting its restart phase — not a
  state-block value). Roots is NOT a carrier (past-window; unrelated
  blocker). Reverted; full diagnosis in [[project_dmc]].
- **Status:** CANONICALIZED (22 occurrences: DMC family-1 rounds
  13 + 19 + 32 + 35 + 36 + 50 + 55 + 60 + 63 + 65 + 70 + 95 + 96 + 101 + 102 + 106 + 108 + 109 + 115 + 117). Canonical form: STATIC opcode probe (read the patched
