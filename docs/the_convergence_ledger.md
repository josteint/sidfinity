...
| TRICHOTOMY VERDICT alignment · rebuild emits its OWN init (universal reset+priming) so streams differ by an init prefix · Check A end-of-init state + aligned play-stream compare · FOUR implementations exist: `verify_cycle._trichotomy_compare` (FC, shift recovery) + `usf_roundtrip._compare_music/_split_aligned` (basic_program, known-init-length + probe search) + the DMC per-IRQ retry + `write_timing_delta._recover_shift` (PER-PLAY-INDEX, and PER CHIP) — TWO recorded CONSULT MISSES, factor at Move 1 | C21 | factor-candidate (4×) |
...
- PRESENTS: an engine whose composer emits the universal reset + typed
  ...
  though play streams match. ⚠ ALSO PRESENTS ONE LEVEL UP: any instrument
  that groups writes by PLAY INVOCATION inherits the same offset in a
  different unit (the rebuild's play 0 is not the original's play 0), and
  the flat Mode-1 verdict is structurally blind to it.
- CANONICAL: Check A (end-of-init chip state) + aligned play-stream compare.
  FOUR implementations exist (`_trichotomy_compare` shift-recovery; basic
  known-length + probe; the DMC per-IRQ retry; `write_timing_delta`'s
  per-play-index, per-chip recovery) and TWO of them were written without
  consulting this entry — consult before writing a fifth. Recovery details
  that are easy to get wrong: |shift| ordering with a strict improvement
  test so 0 wins ties, fall back to 0 on zero matches, and align PER CHIP. The verdict is
...
- MIRROR IMAGE (5th occ, DMC v5 2026-08-25) — the decoder REFUSES what the
  player ACCEPTS, i.e. it manufactures an ERROR rather than a wrong meaning.
  PRESENTS as a batch `error` bucket ("unknown cmd $XX", "never ends",
  IndexError, KeyError), which reads like corrupt data and is actually our
  walk. Three facts to check against the disassembly: an unlisted command byte
  is usually the CMP chain's FALL-THROUGH (a 1-byte no-op — emits nothing, so
  dropping it is lossless); stream POSITIONS are 8-bit registers, so a stream
  with no terminator in its 256-byte window CYCLES and can never "run on" past
  it; and a count INFERRED from the delta between two table addresses is not a
  count at all when the packer relocates those tables independently (C39) —
  bound by reachability (C2).
- ⚠ RELAXING THE DECODER KILLS ANY GUARD WRITTEN AS `except` AROUND IT (one
  took a FULL member down here) — discipline + recipe in
  [`feedback_relaxing_an_error_kills_its_guards`](../.claude/memory/feedback_relaxing_an_error_kills_its_guards.md).
