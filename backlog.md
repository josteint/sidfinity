...
19. DMC v5 TABLE OVERFLOW — 103 members, blocked on a REPRESENTATION decision
    ...
    EXACTLY 256 entries. So the overflow is created by US: the extract
    DE-FUSES the shared table into one captured program per instrument, and the
    original's compactness IS the sharing that de-fusion discards.

    DO NOT RETRY PACKING. Ledger C8 records the 2026-07-01 measurement
    (tmp/measure_suffix_overlap.py): suffix-containment and greedy
    shortest-common-superstring add essentially NOTHING over identical-dedup
    (Deep_Acid 346->346, Speed_Biker 306->305; 2/10 fit). Each program is
    captured as a separate SweepEnvelope with its START prepended, while in the
    original a program's start is just a position that also serves as another
    program's phase byte — so the sequences share no suffixes even when they
    walk the same table.

    THE KNOWN ANSWER, and why I stopped: C8 already names it — EMIT-AND-WALK
    the shared static table (emit the member's own 256-cell table verbatim ONCE,
    point each instrument's wave_ptr/pulse_ptr/filter_ptr into it, like the
    engine). Fits by construction. DMC **v4 already does this** for waves
    (the layout-preserving/positional pool, `wave_table_pos`, ledger C11/C31),
    so there is a form to CONVERGE ON rather than invent. But it needs the USF
    to carry each instrument's ORIGINAL table POSITION = a typed schema
    addition, which CLAUDE.md puts behind the owner gate "even when it passes
