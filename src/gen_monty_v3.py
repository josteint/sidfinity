...
Cloned from gen_commando_v3.py. Pipeline structure identical; only the
SID source path, freq table base, and Lean output names differ. Used
to debug Monty-specific bugs in the V3 player + USF model without
disturbing the byte-perfect Commando pipeline.
...
def main():
  ...
  released := "1985 Gremlin Graphics"
    ...
    with open('src/formal/MontyV3.lean', 'w') as f:
        f.write('\n'.join(out) + '\n')

    print(f"Wrote {len(instruments)} instruments, {len(all_pats)} patterns, "
          f"{len(score['voices'])} voices to MontyV3.lean", file=sys.stderr)
