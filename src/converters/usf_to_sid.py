...
import usf
from usf.format import Song, Pattern, NoteEvent
from sidfinity_pack import pack_sidfinity, ALL_COLUMNS
from converters.gt2_to_usf import pack_wave_left, pack_wave_right, detect_nowavedelay
...
def usf_to_sid(song, output_path=None):
    ...
    for vi in range(3):
        ...
        while i < len(entries):
            ...
            # V2 player repeat logic: byte $Dx causes the pattern to play x
            # times total (counter increments 1..x, replays on 1..x-1, advances
            # on x). So emit $D0+chunk for chunk total plays.
