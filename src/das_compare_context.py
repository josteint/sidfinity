...
def main():
    ...
    nf = int(sys.argv[2]) if len(sys.argv) > 2 else 4000
    ...
    # Find first mismatch at or after start
    first = None
    for fr in range(start, nf):
        if gt[fr] != dm[fr]:
            first = fr
            break
    if first is None:
        print(f"No mismatches in [{start}, {nf})")
        return

    print(f"\nFirst mismatch at frame {first}")
    for fr in range(max(0, first-5), min(nf, first+30)):
        match = "OK" if gt[fr] == dm[fr] else "MISMATCH"
        marker = " <-- FIRST" if fr == first else ""
        ...
        if gt[fr] != dm[fr]:
            ...
            elif gt_only or dm_only:
                if gt_only:
                    print(f"  GT-only: {[(f'{reg_name(a)}=${v:02X}') for a,v in sorted(gt_only)]}")
                if dm_only:
                    print(f"  DM-only: {[(f'{reg_name(a)}=${v:02X}') for a,v in sorted(dm_only)]}")
        else:
            print(f"  BOTH: {gt_s}")
