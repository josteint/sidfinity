"""
codegen_v3.py — V3 player: Hubbard-accurate 6502 code generator.

Based on codegen_v2 but with critical timing changes to match the
Rob Hubbard player's register write behavior:

1. Frequency written directly to SID on note fetch (no wave table delay)
2. Pulse width, ctrl, AD, SR written on note fetch (like Hubbard)
3. Peephole disabled (code size less critical than correctness)

For GT2 songs, use codegen_v2 (the original). V3 is for Hubbard only.

Original codegen_v2 description:
...
class Ctx:
    ...
    def source(self):
        # V3: peephole disabled — correctness over size
        return '\n'.join(self.lines)
...
def emit_effects(ctx):
    ...
    ctx.inst('cmp', '#2')
    # V3: use trampolines for long-range branches
    ctx.inst('bcc', 'ce_fxaddj', comment='FX 1: carry clear → add')
    ctx.inst('beq', 'ce_fxsubj', comment='FX 2: zero → sub')
    # FX 3 or 4 (A > 2)
    ...
    # V3: trampolines for FX dispatch
    ctx.label('ce_fxaddj')
...
def emit_tick0_path(ctx):
    ...
    # V3: use JMP instead of BNE (new note init is too large for branch)
    ctx.inst('beq', 'ce_t0_new')
    ctx.inst('jmp', 'ce_nonew')
    ctx.label('ce_t0_new')
    # Orderlist reader
...
def emit_new_note_init(ctx):
    ctx.section('new note init — V3 Hubbard-accurate')
    ctx.label('ce_newn')
    ctx.inst('sec')
    ctx.inst('sbc', '#NOTE')
    ...
    ctx.inst('lda', 'mt_freqtbllo,y')
    ctx.inst('sta', 'mt_chnfreqlo,x')
    ctx.inst('sta', 'SIDBASE,x', comment='V3: freq_lo to SID immediately')
    ctx.inst('lda', 'mt_freqtblhi,y')
    ctx.inst('sta', 'mt_chnfreqhi,x')
    ctx.inst('sta', 'SIDBASE+1,x', comment='V3: freq_hi to SID immediately')
    ctx.inst('ldy', 'mt_temp2', comment='restore instrument index')
    #
    # 2. AD/SR: write to SID immediately (Hubbard writes AD then SR)
    ctx.inst('lda', 'mt_insad-1,y')
    ctx.inst('sta', 'mt_chnad,x')
    ctx.inst('sta', 'SIDBASE+5,x', comment='V3: AD to SID immediately')
    ctx.inst('lda', 'mt_inssr-1,y')
    ctx.inst('sta', 'mt_chnsr,x')
    ctx.inst('sta', 'SIDBASE+6,x', comment='V3: SR to SID immediately')
    #
    # 3. Vibrato setup
    if ctx.has(VIBRATO):
    ...
    # Write waveform+gate to SID (ctrl register)
    ctx.inst('lda', 'mt_chnwave,x')
    ctx.inst('and', 'mt_chngate,x')
    ctx.inst('sta', 'SIDBASE+4,x', comment='V3: ctrl to SID immediately')
    # Jump to wave table for subsequent processing
