...
def recompile_gt2(sid_path, output_path):
    ...
    # Prefer parse_direct's column data (operand-based, exact) over decompile's
    # sequential walk (which can drift if wave_size estimation is wrong).
    columns = d['columns']
    ni = d['ni']

    # Use parse_direct's column data when available (1-based Y indexing → values)
    rd = r.get('col_data', {})
    def _col(name, default_val):
        """Get column bytes: prefer parse_direct, fall back to decompile."""
        vals = rd.get(name, columns.get(name, [default_val] * ni))
        return bytes(vals[:ni]) if vals else bytes([default_val] * ni)

    ad_col = _col('ad', 0x09)
    sr_col = _col('sr', 0x00)
    wp_col = _col('wave_ptr', 0x00)
    pp_col = _col('pulse_ptr', 0x00) if 'pulse_ptr' in rd or 'pulse_ptr' in columns else None
    fp_col = _col('filter_ptr', 0x00) if 'filter_ptr' in rd or 'filter_ptr' in columns else None
    vp_col = _col('vib_param', 0x00) if 'vib_param' in rd or 'vib_param' in columns else None
    vd_col = _col('vib_delay', 0x00) if 'vib_delay' in rd or 'vib_delay' in columns else None
    gt_col = _col('gate_timer', 0x02) if 'gate_timer' in rd or 'gate_timer' in columns else None
    fw_col = _col('first_wave', 0x09) if 'first_wave' in rd or 'first_wave' in columns else None
    ...
    # Step 6: Read the original PSID header for title/author and binary access
    ...
    # Step 7: Extract default tempo from the binary.
    # extract_default_tempo returns the "audible tempo" (frames per row),
    # but the packer's DEFAULTTEMPO define needs tempo-1 (the value stored
    # in the init code). Subtract 1 to match greloc.c's convention.
    col_data = r.get('col_data', {})
    audible_tempo = extract_default_tempo(
        binary, la, d['code_end'], ni=ni, col_data=col_data, sid_path=sid_path)
    default_tempo = max(0, audible_tempo - 1)

    # Step 8: Detect NOAUTHORINFO from original binary
    noauthorinfo = detect_noauthorinfo(binary, la, d['code_end'])

    # Step 9: Assemble table data from the decompiler.
    # The decompile walks the binary sequentially and splits into tables.
    # Use it directly -- the packer emits tables in the same order.
    wave_left = d.get('wave_left', b'') or b''
    wave_right = d.get('wave_right', b'') or b''
    pulse_left = d.get('pulse_left', b'') or b''
    pulse_right = d.get('pulse_right', b'') or b''
    filter_left = d.get('filter_left', b'') or b''
    filter_right = d.get('filter_right', b'') or b''
    speed_left = d.get('speed_left', b'') or b''
    speed_right = d.get('speed_right', b'') or b''

    # Step 10: Pack using gt2_packer
    sid_bytes, player_size = pack_gt2(
        ...
        pulse_left=pulse_left or None,
        pulse_right=pulse_right or None,
        filter_left=filter_left or None,
        filter_right=filter_right or None,
        speed_left=speed_left or None,
        speed_right=speed_right or None,
