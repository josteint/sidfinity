...
def recompile_gt2(sid_path, output_path):
    ...
    # Step 4b: Override FIXEDPARAMS and SIMPLEPULSE with binary-detected values.
    # The data-derived values can disagree with what the original binary was
    # compiled with (e.g. pulse table data has 0x80 "set" entries which look
    # like SIMPLEPULSE=0, but the binary was actually compiled with SIMPLEPULSE=1
    # where the set-pulse path only writes lowbyte). Trust the binary detection.
    if version_info:
        if version_info.get('fixedparams') is not None:
            flags['FIXEDPARAMS'] = version_info['fixedparams']
        if version_info.get('simplepulse') is not None:
            flags['SIMPLEPULSE'] = version_info['simplepulse']
    ...
    # Step 6: Extract DEFAULTTEMPO using robust 3-strategy approach.
    # Strategy 1: pattern-match init code, Strategy 2: greloc.c last-instrument
    # formula, Strategy 3: validate via siddump first-note timing.
    from gt2_tempo import extract_default_tempo
    from gt_parser import parse_psid_header as _parse_header
    with open(sid_path, 'rb') as _f:
        _sid_data = _f.read()
    _hdr, _bin, _la = _parse_header(_sid_data)
    col_data_for_tempo = d['columns']
    default_tempo = extract_default_tempo(
        _bin, _la, d['code_end'], ni, col_data_for_tempo, sid_path) - 1
