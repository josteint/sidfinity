...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 ~5378/5401 FULL (99.6%); residue = C19 singleton wedges + off-table/CIA hard residue. Full 2SID/3SID (C27/C28) + compilations incl. HETEROGENEOUS ([[project_dmc_compilations]]). r162 closed Vegeta/Trzewiki (periodic-COUNTER play-repeat wrapper: 4 bodies/IRQ + 1 every 41st → uniform `P4×40+P5` play_phases schedule, reuses composer Pn, ledger **C24** 6th form); r161 closed Vegeta/Heniek (SMC-immediate PARITY play-repeat wrapper 1/3 → `P_P3` schedule, composer `P2`→`Pn`, ledger **C24** 5th form). **READ project_dmc.md FIRST** — round history + every lever + methodology live there (newest-first); do NOT expand this line (new rounds → topic file). Coverage = a FRESH batch (corpus code_hash stale pending next); re-verify any stale wide batch before picking.
