...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 family-1 **≈5169/5401 FULL, ≈232 partial, 0 unsupported/error (per-round; wide batch STALE)**; family-2 2507/2889; V5 fam-3/5 1098/1495; fam-4 in progress. Full 2SID/3SID support (each chip = an independent single-chip player; merged log chip-TAGGED reg=chip*$20+reg; verdict compares each chip's stream independently — ledger C27/C28). Per-SID levers land as first-f1-partial-by-path fixes: extract-time derivation knobs or gated composer params, positive-minority census, 0-regr-by-construction, ledger-recorded; latest r69 Enter/Bax = a PER-MEMBER IN-TABLE vibdepth deviation (`$1888[44]=$10` vs canonical $20 = a note authored to vibrate half as deep; read as vstep=vibdepth[curnote] at note-init via the glide start-note → orig vibrato step $10 vs reb $20 = 2×); FIX = generalize the round-66 code-overlap-HEAD capture gate `n<6 and mem!=VIBDEPTH[n]` → `n<96` in `_assign_offtable_freq.add_note` (capture any REACHABLE note whose vibdepth byte differs from canonical; composer's existing `_vd[n]` in-place override handles it; 0-regr by construction — canonical members deviate nowhere they play; ledger C11 vibdepth head→in-table). LESSON: the vibdepth table is per-member MUSICAL content — the head was the special case, not the whole class. r68 Secret_Loser = the NOTE-FETCH/glide-arrival base-freq reloads (`ev_note`/`fx_gl_chk`) read `freqlo[curnote]` RAW, ignoring the LIVE off-table redirect the wave-step already honored; FIX = shared `reload_base` subroutine jsr'ed from both sites (ledger C11). LESSON: an off-table freq index has THREE read sites — every site must honor a captured LIVE record. NB the Jul-9 wide batch (dmc_f1_dedup.jsonl) is STALE — re-verify partials before picking; many leading Bakewell partials already flipped FULL (rounds 55–68). **Per-round history (rounds 1–68) + every lever + methodology lessons live in project_dmc.md — READ IT FIRST.**
