...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — reloc-invariant fingerprinting (`pipelines/future_composer/engine_fingerprint.py`); 91% of HVSC FC is ONE "standard" player. **2,572 FULL of 4,093 enumerated** ⚠STALE-VERDICTS since 2026-09-01 (backlog 31's key fix; byte-neutral — 6/6 byte-identical spot check). Full re-batch 2026-08-31, 0 regressions member-for-member. The former "2,604" counted 32 members #85 REMOVED (backlog 37); 47 dead pre-#85 rows remain in the store. Per-round detail in topic file — READ IT first.
...
- [GoatTracker family](project_goattracker.md) — 2nd-largest (8,670). **Active: V1** (original 1.x, NOT GT2). Extract + composer built; wide batch **168/1387 FULL** — verdicts CURRENT: goattracker_v1 is the ONE family backlog 31's fix did not move (it has no regression portfolio file). Full re-batch 2026-08-31: 1387/1387 identical, 0 status changes. Re-run THROUGH the stored `.usf`; wired into regression.py. Detail in topic file — READ IT first.
