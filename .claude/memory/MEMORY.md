...
- [Hubbard remaining partials](project_hubbard_remaining_partials.md) — after Monty 19/19, 6 Hubbard subtunes still write-log-partial: Human_Race(4)[+prob Battle(1)] = spurious bidir PWM (composer zeros v_pwperiod/v_pwdir that the engine seeds from binary $0DC8; root found via pc-trace, CIA was a red herring); Devils_Galop(1) = dropped V3 freq write (separate). All false-passed by the old snapshot verdict.
