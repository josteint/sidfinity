...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs). V4 family-1 **4829/5401 FULL (89.4%, round 29)**; family-2 2413/2889 (83.5%); V5 fam-3/5 1088/1495; fam-4 in progress. Residue ~357 partial = positional + in_table/off-table freq drift + genuinely-varying off-table reads + tail. LESSONS: a "regressing" correct fix usually hits a STALE baseline/batch-flake/suboptimal-path FULL — re-baseline vs a FRESH single-member build ([[feedback_convergence_ledger]] C20); a play-body JSR redirected to an N-call stub = play-body UNIT-repeat, unified `play_unit_repeat` list (rounds 26/26b, ledger C24); an off-table read on a globally-varying byte that's STABLE-AT-THE-READ (sectorpos $1729) is fixable by EVENT-DRIVEN capture — but only on WINDOW-served (non-redirect-mapped) idx, else it breaks the round-25 seed (round 27, ledger C11; the /amend skill overturned the "sectorpos positional" blanket). Per-round history + methodology in topic file — READ IT first.
