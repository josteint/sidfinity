...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs). V4 family-1 **4803/5401 FULL (88.9%, round 26b)**; family-2 2413/2889 (83.5%); V5 fam-3/5 1088/1495; fam-4 in progress. Residue ~383 partial = positional + in_table/off-table freq drift + $1720-2 unmappable + tail. LESSONS: a "regressing" correct fix usually hits a STALE baseline/batch-flake/suboptimal-path FULL — re-baseline vs a FRESH single-member build ([[feedback_convergence_ledger]] C20); a redirect-mapped var that doesn't init-track (sparse glide state) needs leftover-SEEDING not removal (round 25, ledger C11); a play-body JSR redirected to an N-call stub = play-body UNIT-repeat, unified `play_unit_repeat`=[v0,v1,v2,filter] list via static byte-probe (rounds 26/26b, ledger C24 — CORE TENET governs config fields, not the §7 musical lens). Per-round history + methodology in topic file — READ IT first.
