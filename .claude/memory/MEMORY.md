...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC fingerprinting). 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage target. Migration in `pipelines/future_composer/standard/` (rep Carter/Jarre_2). The standard player is a DIFFERENT ENGINE from the Tel composer (pattern/instrument/effect/write-model all differ). DONE + gated (canaries 15/15): base + standard PATTERN decoder + instrument-select fix + $40 effect (= wave_arp reuse) + $80 effect (= noise_tick reuse, nt_flag order trick) + PULSE sweep (commit 23c9f22; corrected bands, fx2&7 selector overlapping the fx2&$FC default, explicit branch for the orig's OOB-read quirk; acc=d402/d403+pulsetest, no new state). V2 exact; match prefix 11; f1 aligns through V3+V2+V1-PW. **RESUME: wave-program FREQ part** (absolute mode confirmed live: freqtab[clk-1]+$0D, sel1 $24→$31), then the $D416=$FF default write ($1C2F path), vibrato (+5=$27), counter init-phase, relocation. Adrenalin engines are custom outliers (deprioritized).
