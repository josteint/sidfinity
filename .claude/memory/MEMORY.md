...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs). V4 family-1 **4802/5401 FULL (88.9%, round 26)**; family-2 2413/2889 (83.5%); V5 fam-3/5 1088/1495; fam-4 in progress. Residue ~384 partial = positional + in_table/off-table freq drift + $1720-2 unmappable + tail. LESSONS: a "regressing" correct fix usually hits a STALE baseline/batch-flake/suboptimal-path FULL — re-baseline vs a FRESH single-member build ([[feedback_convergence_ledger]] C20); a redirect-mapped var that doesn't init-track (sparse glide state) needs leftover-SEEDING not removal (round 25, ledger C11); a play-body JSR redirected to a double-call stub = per-voice tick MULTIPLIER, `voice_tick_repeat` param via static byte-probe (round 26, ledger C24 NEW). Per-round history + methodology in topic file — READ IT first.
