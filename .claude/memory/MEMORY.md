...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,758 SIDs in HVSC #85; largest family). V4 f1: **5,401/5,401 FULL (100%)** (2026-08-04, r180 — Lane_Crazy closed via C8 pooled pulse-step layout + position-observability dedup key). Full 2SID/3SID (C27/C28) + compilations incl. HETEROGENEOUS ([[project_dmc_compilations]]). r164-166 closed Verdict/Verdict_01 FULL (the C19 "$FF→init RESTART, resume shape" wedge — a mid-song restart whose ghost frame garbage-indexes tables; solved in 3 layers: ghost-machinery restart burst + `glide_offtable` C6 redirect + `irecimg` 11-byte-record image for the garbage pulse-step read; re-test challenge disproved a "hard boundary" mis-claim 3×). **READ project_dmc.md FIRST** — round history + every lever + methodology live there (newest-first); do NOT expand this line (new rounds → topic file). Coverage VERIFIED 2026-08-08: fresh f1 batch 5,401/5,401 FULL, corpus mass-written + audited (10/10 from disk); the P1 `speed_steps`/P3b `step_base` convergences landed with it. ⚠ those rows are code_hash-STALE since the 2026-08-09 hvsc85 rename (docstring-only edits in fingerprinted files moved dmc_v4's hash), so `dmc_mass_write` refuses them until f1 is re-batched — the VERDICTS are still musically valid, only the fingerprint moved (ledger C20 seventh layer).
