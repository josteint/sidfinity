...
- [Basic_Program family](project_basic_program.md) — 524 RSID-BASIC tunes on #85 (engine IS the BASIC+KERNAL ROM); trace-lift → USF round-trip. **488/524 FULL + Cascading recovered 2026-08-28** (the pw-sweep portfolio carrier, FULL at its honest 288s window — three stale caps, not a representation gap; Legion/Pepper now real divergences). ⚠STALE-VERDICTS (all rows — the 2026-08-28 edits; re-batch queued behind the DMC chain). Residue leads + round history: topic file.
