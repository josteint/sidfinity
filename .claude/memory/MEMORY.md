...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC fingerprinting). 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage target. Migration in `pipelines/future_composer/standard/` (rep Carter/Jarre_2). The standard player is a DIFFERENT ENGINE from the Tel composer (pattern/instrument/effect/write-model all differ). DONE + gated (canaries 15/15): base + standard PATTERN decoder + instrument-select fix + $40 effect (= wave_arp reuse) + $80 effect (commit 13beae3; = noise_tick reuse, style='standard' — V3 attack+restore EXACT incl. the hi,lo/lo,hi order asymmetry via nt_flag). V1 wave-program ctrl exact. Writes 13494/17189. **RESUME: the PULSE sweep** (orig V2 +$40/frame, V3 +$C0/frame = fx2&$FC default step; the spliced standard pulse emitter isn't producing it), then vibrato (+5=$27, $1A0A-$1B3F), filter $D416, wave-freq, counter init-phase, relocation. Adrenalin engines are custom outliers (deprioritized).
