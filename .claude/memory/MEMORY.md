...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 family-1 **≈5169/5401 FULL, ≈232 partial, 0 unsupported/error (per-round; wide batch STALE)**; family-2 2507/2889; V5 fam-3/5 1098/1495; fam-4 in progress. Full 2SID/3SID support (each chip = an independent single-chip player; merged log chip-TAGGED reg=chip*$20+reg; verdict compares each chip's stream independently — ledger C27/C28). Per-SID levers land as first-f1-partial-by-path fixes: extract-time derivation knobs or gated composer params, positive-minority census, 0-regr-by-construction, ledger-recorded; latest r68 Secret_Loser = the NOTE-FETCH/glide-arrival base-freq reloads (`ev_note`/`fx_gl_chk`) read `freqlo[curnote]` RAW, ignoring the LIVE off-table redirect the wave-step already honored → off-table curnote $F4 read the static ovrwin byte instead of live $173B (V1 dur counter); FIX = shared `reload_base` subroutine running the SAME `_gen_offtable_redirect`, jsr'ed from both sites (0-regr: 17/17 affected-path FULLs + 5 CIA FULLs hold; ledger C11). LESSON: an off-table freq index has THREE read sites — every site must honor a captured LIVE record. NB the Jul-9 wide batch (dmc_f1_dedup.jsonl) is STALE — re-verify partials before picking; many leading Bakewell partials already flipped FULL (rounds 55–68). **Per-round history (rounds 1–68) + every lever + methodology lessons live in project_dmc.md — READ IT FIRST.**
