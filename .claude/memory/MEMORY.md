...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC version fingerprinting). Found 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage FC target. Migration started in `pipelines/future_composer/standard/` (rep Carter/Jarre_2): disasm+address-mapped, config+extract WORK; next = map aux effect tables (arp/pulse/filter/drum) then write-log iterate. Adrenalin's engines are fingerprint-confirmed CUSTOM outliers (no shortcut; deprioritized).
