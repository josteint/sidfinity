...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 **5347/5401 FULL (99.0%) + 54 partial + 0 error** (r129-r133 2026-07-29: wave in-table-runaway C2 +2, CIA latch cross-check C9 +2, per-subtune speed mask C9 +1, swing-driver latch median C9 +3, cymbal-ctrl wedge C19 +1 vs the r128c batch; corpus synced r128c — code_hash now stale pending next fresh batch). Full 2SID/3SID (C27/C28) + compilation support incl. HETEROGENEOUS ([[project_dmc_compilations]]). **Round history + every lever + methodology live in project_dmc.md — READ IT FIRST** (do NOT expand this line; new rounds → topic file). Re-verify any stale wide batch before picking.
