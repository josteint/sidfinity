...
- [Hubbard remaining partials](project_hubbard_remaining_partials.md) — RESOLVED: CIA-aware per-play verdict implemented; Human_Race 5/5 + Battle 1/1 PASS. verify_all routes CIA subtunes (PSID speed!=0) through siddump --writelog-per-irq (cycle-base-corrected, frame-0 init drop) + flat compare. Hubbard now 70 ok + 1 regressed = Devils_Galop(1), a real dropped-V3-freq bug (vblank, still open/separate).
