...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs). V4 family-1 **≈5163/5401 FULL, ≈238 partial, 0 unsupported, 0 error (per-round; wide batch STALE)**. Latest per-SID levers (all first-f1-partial-by-path, 0 regr, ledger-recorded): r65 Groove = `$D418` RE-ASSERTED EVERY FRAME via a filter-tail JSR-wrapper (COMPOSER param `d418_filter_tail`; note-init stores mode to a `d418mode` shadow + suppresses its $D418, filter tail re-asserts `lda d418mode/ora mvol/sta $d418`; C19 10th / C10 master-vol-every-frame). TRAP: the first LOOSE byte-probe false-fired on Qbhead_01's aux routine ($1CA8) and would have regressed a FULL — a STATIC opcode probe must anchor on the REACHABLE play-body site, and verify each census carrier's first-div before committing. r64 Attacker = RESET-ALL loop target can be PER-VOICE not one N (`loop_reset_pos` scalar→tuple, anchored to track_pos; C13, extract-only); r63 Sans_intro init-prefix forced-subtune (C19, extract-only); r62 Action_G reset-all-to-N (C13, extract-only). Round 48 built full 2SID/3SID support (ledger C27 NEW): a multi-SID tune = N independent single-chip players run sequentially, so the merged write-log = [chip1][chip2] and each sub-player uses the existing machinery chip-TAGGED (siddump reg=chip*$20+reg; USF voices number through chips, chip ADDRESSES are pipeline constants $D420/$D440 NOT in USF, wired to the verdict). Nice_Dream_2SID unsupported→partial — **EVERY f1 member now at least partial**. Round 49 (ledger C28 NEW) fixed the multi-SID VERDICT: compare each chip's stream INDEPENDENTLY (cross-chip write order is physically UNobservable — Trap-B analogue); the round-48 "residual = filter-res-timing at write 3221" was a MISDIAGNOSIS (a cross-chip ordering artifact of comparing the merged stream flat). `compare_instruction_stream(n_chips=N)` splits by reg//0x20; single-chip byte-identical, regression green. Nice_Dream match 3221→63496; TRUE blocker now a single-chip note-duration drift at ~74s (freq-drift tail, RE_NOTES bucket 9). TRAP: short/pc-trace captures showed "byte-perfect" — verify at FULL songlength before declaring FULL. Round 47 solved the init-unpacker class (ledger C26 NEW: Flash trio = init GENERATES all data tables in high RAM, all operands out-of-image → extract from post-init RAM via data_post_init; + Itinerant banking-wrapper JT-less base from the wrapper JSR target). SOLE remaining unsupported: Surgeon Nice_Dream_2SID = two full 2entry player instances → needs second-chip support (shared with round-45 2SID partials). Round 46 emptied ALL other unsupported buckets: no_jumptable 62→0 (restructured-init locate fallbacks: all-occurrence `_sigs_op`, inner-shape tunetab/wavectrl/d417/secp anchors, tiered wrapper-JMP base candidates, header-play retrace; ledger C13 note) + cascade over pcm/instr_base/loop_site (+24 FULL) + rest_effects='vibflip' singleton; round 45: error cluster cleared 25→0 + sector_decode emptied (8-bit track/sector wrap, C11). 271 partials = staged-conversion residue + freq-drift/otrk_legacy tail. Family-2 2507/2889 (86.8%); V5 fam-3/5 1098/1495; fam-4 in progress. LESSON (round 39, ledger C11): "no composer var to redirect to" is usually a one-edit materialization (rename the scratch at the writer site), not a rejection; round-trip-verify a RECONSTRUCTED value (iflags) per instrument before mapping it. LESSON (round 35): a hack executing garbled/illegal opcodes → STOP hand-simulating; pc-trace + Python-simulate the generator vs the observed stream (3826/3826) BEFORE composing. LESSON (round 34, ledger C19 corollary): a probed knob must be honored on EVERY orig path funneling through the patched site (the rest-skip JMP also covers $7C soft-note fetches). LESSONS: a "regressing" correct fix usually hits a STALE baseline/batch-flake/suboptimal-path FULL — re-baseline vs a FRESH single-member build ([[feedback_convergence_ledger]] C20); a play-body JSR redirected to an N-call stub = play-body UNIT-repeat, unified `play_unit_repeat` list (rounds 26/26b, ledger C24); a "positional" byte counter whose per-event deltas derive from row kind + STATED commands is shadowable live — stated-command fx_flags (dcmd/icmd/vcmd/softcmd) = §8 arrangement, the round-33 sectpos fix that overturned the round-22 blanket (ledger C11). Per-round history + methodology in topic file — READ IT first.
