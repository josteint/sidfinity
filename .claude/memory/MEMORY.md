...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 ~5378/5401 FULL (99.6%); residue = C19 singleton wedges + off-table/CIA hard residue. Full 2SID/3SID (C27/C28) + compilations incl. HETEROGENEOUS ([[project_dmc_compilations]]). r159 closed Bomberman_preview (CONDITIONAL per-subtune song remap: init wrapper sends only sub 0 → song 5 → observed `subtune_songs` map, ledger **C19** 38th occ); r158 closed Surgeon/Mothafucka_2SID (RELOCATING WRAPPER multi-SID → post-init RAM for every chip, ledger **C27** 4th occ). **READ project_dmc.md FIRST** — round history + every lever + methodology live there (newest-first); do NOT expand this line (new rounds → topic file). Coverage = a FRESH batch (corpus code_hash stale pending next); re-verify any stale wide batch before picking.
