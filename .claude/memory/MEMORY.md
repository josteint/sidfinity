...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC fingerprinting). 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage target. Migration in `pipelines/future_composer/standard/` (rep Carter/Jarre_2). The standard player is a DIFFERENT ENGINE from the Tel composer (pattern/instrument/effect/write-model all differ). **✅ JARRE_2 SUB 0 FULL (2026-06-10): play 17164/17164, trichotomy audio✓ — the standard player's first verified tune.** Whole chain done + gated (canaries 15/15): pattern decoder, $Cx instrument-select, $40=wave_arp, $80=noise_tick (fw_mode write-order dispatch), pulse sweep, wave ctrl+freq (relative = semitone ARP), VIBRATO (triangle, semitone-delta step w/ faithful quirks, direct write ctr>=4), $D416 filter-off default, tick gate-off ($19FA), note-duration raw+1 (extract), init_style='universal_reset' (orig init = pure zeroing, no priming). **RESUME: ear-test Jarre_2 + RELOCATION → one config for the 3673-SID family**; unimplemented (surface via write-log on family SIDs): filter bit-SET, $Ex glide, +$B0 pulse jitter, $2030/bit2 effect. Adrenalin engines are custom outliers (deprioritized).
