...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 family-1 **≈5169/5401 FULL, ≈232 partial, 0 unsupported/error (per-round; wide batch STALE)**; family-2 2507/2889; V5 fam-3/5 1098/1495; fam-4 in progress. Full 2SID/3SID support (each chip = an independent single-chip player; merged log chip-TAGGED reg=chip*$20+reg; verdict compares each chip's stream independently — ledger C27/C28). Per-SID levers land as first-f1-partial-by-path fixes: extract-time derivation knobs or gated composer params, positive-minority census, 0-regr-by-construction, ledger-recorded; latest r71 Balloonacy/Bayliss = a 4-player COMPILATION unblocked by two compilation-path-only fixes (ledger C31/C8): region-bounded per-player `dataflow.locate(region=(base,base+0x900))` — a co-packed player keeps dead-code cross-player JMPs so the static trace bleeds into the sibling → every signature matches twice → ambiguous None; + offtable-union instrument dedup (`merge_models` drops `offtable_freq`, a C6 reachability artifact, from the dedup key + UNIONs records) to fit the 28-inst 5-bit cap (29→28). Both merge/base_override-only → 0-regr on single-player + all FULL compilations. r70 Feed_a_Bird/Bax = a C19 hand-patched wedge in the tie/legato SWITCH ($7D) handler: it toggles the gate mask (`$100f,x`) with `EOR #imm` — canon $01 (flip gate bit, $FF↔$FE = release) patched to `$1F` at base+$18D so a SWITCH flips gate+test+ring+sync+triangle ($FF↔`$E0`), CUTTING a triangle/ring/sync note to silence (`$17&$E0=$00` vs our `$17&$FE=$16`); FIX = `factory._switch_toggle_mask_probe` (static opcode probe) → USF `switch_toggle_mask` → composer `ev_switch` emits `eor #<mask>` (default $01 byte-identical; 1 carrier / 5502 canon / 0 FULL exposure; ledger C19 11th occ). LESSONS: (a) an EOR/AND IMMEDIATE-only wedge (opcode+operands unchanged) is INVISIBLE to dmc_canon_diff — find it via the divergence recipe + a memwatch of the exact state byte across the diverging write; (b) the disasm header's list of a state byte's values ($FF/$FE) may be INCOMPLETE — trust the runtime memwatch ($1011 went →$E0). r69 Enter/Bax = a PER-MEMBER IN-TABLE vibdepth deviation (`$1888[44]=$10` vs canon $20); FIX = generalize the head capture gate `n<6` → `n<96` in `_assign_offtable_freq.add_note` (ledger C11/C6 vibdepth head→in-table). r68 Secret_Loser = base-freq reloads must honor the LIVE off-table redirect at ALL THREE read sites (wave-step / note-fetch / glide-arrival) via a shared `reload_base` (ledger C11). NB the Jul-9 wide batch (dmc_f1_dedup.jsonl) is STALE — re-verify partials before picking; many leading Bakewell partials already flipped FULL (rounds 55–68). **Per-round history (rounds 1–71) + every lever + methodology lessons live in project_dmc.md — READ IT FIRST.**
- [DMC compilations](project_dmc_compilations.md) — a WHOLE residue class: one file packs N independent DMC players + a per-subtune SMC dispatch wrapper (subtune→(base,song)). Ledger C31; ≥15 in Bayliss alone. Abyssal_Karma is the first; fix (unified-merge vs dual-player) DECISION PENDING. Distinct from C27/C28 multi-SID.
