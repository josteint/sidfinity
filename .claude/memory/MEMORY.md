...
- [Hubbard remaining partials](project_hubbard_remaining_partials.md) — after Monty 19/19: Human_Race(4)[+prob Battle(1)] ENGINE IS CORRECT (per-play write seq byte-identical to orig, 54/54 via pc-trace); they fail only because CIA-timed + siddump flat capture buckets init/first-play out of phase → fix the VERDICT (verify CIA tunes per-play), not the engine. (3 buggy-trace-parsing dead-ends: CIA-dispatch, PWM-seed — both wrong.) Devils_Galop(1) = real dropped-V3-freq bug (vblank, separate).
