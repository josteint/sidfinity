...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 **5340/5401 FULL (98.9%) + 61 partial + 0 error** (r129 2026-07-29: wave in-table-runaway C2 fix, Long_Time + Sweet_Remix +2 vs the r128c batch; corpus synced r128c — code_hash now stale pending next fresh batch). Full 2SID/3SID (C27/C28) + compilation support incl. HETEROGENEOUS ([[project_dmc_compilations]]). **Round history + every lever + methodology live in project_dmc.md — READ IT FIRST** (do NOT expand this line; new rounds → topic file). Re-verify any stale wide batch before picking.
