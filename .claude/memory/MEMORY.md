...
- [Music_Assembler](project_music_assembler_target.md) — **IN PROGRESS** (6,351 SIDs, the largest un-migrated family). Locate + full packed-format decode + model + composer; **Sid_Slam FULL (85,574 writes), 15/60 sampled members FULL with no per-member work**. Dominant residue = ledger C6 off-table freq. Not yet through USF. **Freespace_2075 (the one DMC f1 member packing MA players) rebuilds FULL on all 3 subtunes** via heterogeneous.py, pipeline wiring still pending. Also unblocks `Freespace_2075`.
