...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 family-1 **≈5169/5401 FULL, ≈232 partial, 0 unsupported/error (per-round; wide batch STALE)**; family-2 2507/2889; V5 fam-3/5 1098/1495; fam-4 in progress. Full 2SID/3SID support (each chip = an independent single-chip player; merged log chip-TAGGED reg=chip*$20+reg; verdict compares each chip's stream independently — ledger C27/C28). Per-SID levers land as first-f1-partial-by-path fixes: extract-time derivation knobs or gated composer params, positive-minority census, 0-regr-by-construction, ledger-recorded; latest r67 Toccata_v2 = play-vector wrapper's R phase runs the PULSE TAIL ($135D, a 2nd pulse advance/tick from the STALE $171F step temp), mis-observed as a P_R123 register refresh → `rphase_variant='pulse_tail'` execution-probe (watch for JSR base+$35D) + composer `pulse_tail` routine (C18 R-entry variant, twin of vibflip); census over all 743 non-canonical-play f1 = 1 carrier. NB the Jul-9 wide batch (dmc_f1_dedup.jsonl) is STALE — re-verify partials before picking; many leading Bakewell partials already flipped FULL (rounds 55–66). **Per-round history (rounds 1–67) + every lever + methodology lessons live in project_dmc.md — READ IT FIRST.**
