...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 **5256/5401 FULL + 145 partial + 0 error** (r90 batch; r91-100 gained Rogue_Ninja + Super_Seven + Super_Tau-Zeta + Black_It + Ice_on_Fire + the r96 loop-to-N seven + Cleve_24 + Object_of_Art + Assassins + Useless_1994 + Dance + Experiment + Classic_Mix + Bassy_Introtune + Blue_Dos_t0nt + Two_Channels + Trailways_A + Cliche_Beat (+Dr_Piotr ×2 confirmed), not yet re-batched). Full 2SID/3SID (C27/C28) + compilation support incl. HETEROGENEOUS ([[project_dmc_compilations]]). **92-round history + every lever + methodology live in project_dmc.md — READ IT FIRST** (do NOT expand this line; new rounds → topic file). Re-verify any stale wide batch before picking.
