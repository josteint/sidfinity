...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC fingerprinting). 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage target. Migration in `pipelines/future_composer/standard/` (rep Carter/Jarre_2). The standard player is a DIFFERENT ENGINE from the Tel composer (pattern/instrument/effect/write-model all differ). DONE + gated (canaries 15/15): base + standard PATTERN decoder + instrument-select fix + $40 effect (= wave_arp reuse) + $80 effect (= noise_tick reuse) + PULSE sweep + wave-program FREQ (relative = semitone ARP; fw_mode 3-state freq-write dispatch) + $D416 filter-off default (filtvoice latch) + standard tick GATE-OFF ($19FA; the 'counter init-phase' theory was WRONG — it was this). **FRAMES 1-3 EXACT, match prefix 46** (commits …4c0a355, e18bda0). **RESUME: VIBRATO** (f4 divergence: V3 freq base+$0C lo,hi direct BEFORE PW; chain seg $1A0A-$1B3F; inst +5=$27 param; RE then emit direct write, don't touch shadows). Then re-check note timing, filter bit-SET, relocation. Adrenalin engines are custom outliers (deprioritized).
