...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1: fresh batch 2026-08-04 = **5,832/5,833 FULL (99.98%)**; the 1 partial = Lane_Crazy (phase-4 wave-pool identity > instr cap, fix designed awaiting review). Full 2SID/3SID (C27/C28) + compilations incl. HETEROGENEOUS ([[project_dmc_compilations]]). r164-166 closed Verdict/Verdict_01 FULL (the C19 "$FF→init RESTART, resume shape" wedge — a mid-song restart whose ghost frame garbage-indexes tables; solved in 3 layers: ghost-machinery restart burst + `glide_offtable` C6 redirect + `irecimg` 11-byte-record image for the garbage pulse-step read; re-test challenge disproved a "hard boundary" mis-claim 3×). **READ project_dmc.md FIRST** — round history + every lever + methodology live there (newest-first); do NOT expand this line (new rounds → topic file). Coverage = a FRESH batch (corpus code_hash stale pending next); re-verify any stale wide batch before picking.
