...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 **5237/5401 FULL + 164 partial + 0 error** (r83 batch +Defuzion_3; corpus mass-written at r82, not yet re-synced for r83). r83: the COMPILATION path's three defects — dispatch OBSERVED not parsed, record 0 restored to merged slot 0, idle priming per-subtune (r81-82: multi-SID sub-player constructor runs the full canonical build; per-STORE keep_regs; **the mass-writer was storing artifacts the batch never verified — C20 4th layer**); f2/V5 fam-3/5 in progress. Full 2SID/3SID (C27/C28) + compilation support incl. HETEROGENEOUS (DMC players + a distinct `dmc_sfx` SFX sub-player, ledger C31 — see [[project_dmc_compilations]]). Per-SID levers = first-f1-partial-by-path fixes: gated params, positive-minority census, 0-regr-by-construction, ledger-recorded. **72-round history + every lever + methodology live in project_dmc.md — READ IT FIRST** (do NOT expand this line; new rounds → topic file). Re-verify any stale wide batch (dmc_f*.jsonl) before picking.
