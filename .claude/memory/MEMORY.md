...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs, largest family). Census: V4 canonical = 5401 (50.6%) = the target. **✅✅ FAMILY-1: 2945/5401 FULL (54.5%, 2026-06-13)**, mass-written + db-refreshed; Zaks ear-test passed. Pipeline complete: dataflow-operand extract + path-resolved patterns → OUR composer engine → xa65, factory `dmc_v4_config` + wide runner `tools/dmc_family_batch.py`. Solved this run: gate-mask/dual-phase/$D417 leftovers as priming, loop-to-target variant, per-tune freq tables, idle wave program, off-table window, 16-bit pattern ptr. NEXT (RE_NOTES.md residue): relocation classes (~1800, FC-reloc-factory analogue) → other sub-builds → family 2 (2889) → V5.
