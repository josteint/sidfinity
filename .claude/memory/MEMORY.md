...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 **5368/5401 FULL (99.4%) + 33 partial + 0 error** (r129-r146 2026-07-30: wave in-table-runaway C2 +2, CIA cross-check/speed-mask/swing-median C9 +6, cymbal-ctrl/$FF-stub/transpose-bias wedges C19 +3, parity-wrapper P2 phase C24 +1, Deprave live-stack full overturn C29 +1, forced-record JSR-form wedge C19 +1, C37-observation resume wrapper +1, play-clock-in-sector C19 +1, C18 static-table refine +1, SID-mirror pw-base C19 +1, wavestep-arm C18 +2, $D418-every-play indirect-topology wrapper C19 +1, F-phase per-voice repeat massive-multispeed C18 +1, CIA per-play latch re-arm C25-mirrored +1; r147 compilation detection for NON-page-aligned lo/hi-pair wrapper C31 +0; r148 PER-SUBTUNE idle_wave C31 (Pievspie/Mission_Moon FULL, +1); r149 per-instrument record_offset C31/C11 (sonified ioff survives the merge renumber, Pinov_Vox/Goldrake FULL, +1); r150 TIME-MEDLEY C31 variant (Praiser/Mega_Mix FULL + productionized: play-vector countdown dispatch, per-segment shadow17 save/restore, sole carrier in 10,676, +1); r151 SPLIT-RELOCATION curnote detection C13 (note-fetch-STA cross-check, Psych858o/Pulsate FULL, 6 carriers all FULL, +1); r152 Rayden DURREL-RAMP driver C19 EXTRACT-ONLY DECONSTRUCTION (global period-4 note-duration beat → per-row `duration`, NOT a composer param = §8 leak avoided; +3, 2 $8x carriers byte-identical); r153 POST-NOTE GUARD immediate wedge C19 34th occ (Rayden/NOFX_tune_2: note-init `LDA #$02`→`#$00` = gate drops 1 frame after note-init not 3 → composer `note_guard_init`, sole carrier, +1); r154 PULSE UP-REVERSAL bound repoint C19 35th occ (Rygar/Complications: canon `CMP $1759,x` bound-B → `CMP $1710,x` route-bit const at $1393 = PW up-sweep ramps full 16-bit range → composer `pw_up_reverse='routebit'`=`cmp fbit,x`, 2 carriers Complications partial→FULL + Hexen_Remake stays FULL, +1); r155 STATIC $D418 C19 36th occ (Signor/Logic_Intro: both canon $D418 stores NOPed + appended init-wrapper writes $D418=$3F ONCE = no play-time $D418 → composer `master_vol_static=$3F`, sole carrier, +1) vs the r128c batch; corpus synced r128c + Mega_Mix — code_hash stale pending next fresh batch). Full 2SID/3SID (C27/C28) + compilation support incl. HETEROGENEOUS ([[project_dmc_compilations]]). **Round history + every lever + methodology live in project_dmc.md — READ IT FIRST** (do NOT expand this line; new rounds → topic file). Re-verify any stale wide batch before picking.
