...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC fingerprinting). 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage target. Migration in `pipelines/future_composer/standard/` (rep Carter/Jarre_2). The standard player is a DIFFERENT ENGINE from the Tel composer (pattern/instrument/effect/write-model all differ). Base pieces (instrument decode→vibrato-fix, vol-first, conditional-freq) + wave-ctrl + standard PATTERN decoder DONE + gated (canaries 15/15; commit 1af4a15). The "wrong notes" premise was a mis-diagnosis — pitches coincided; the real fix was $Cx instrument-select. **RESUME: the per-frame EFFECT CHAIN** (frame-1 blocker: $40 effect $1E32 decoded, $80 effect, wave-freq, pulse, filter). Adrenalin engines are custom outliers (deprioritized).
