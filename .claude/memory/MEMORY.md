...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 family-1 **≈5169/5401 FULL, ≈232 partial, 0 unsupported/error (per-round; wide batch STALE)**; family-2 2507/2889; V5 fam-3/5 1098/1495; fam-4 in progress. Full 2SID/3SID support (each chip = an independent single-chip player; merged log chip-TAGGED reg=chip*$20+reg; verdict compares each chip's stream independently — ledger C27/C28). Per-SID levers land as first-f1-partial-by-path fixes: extract-time derivation knobs or gated composer params, positive-minority census, 0-regr-by-construction, ledger-recorded; latest r66 Journey = note+transpose WRAPS off-table (8-bit ADC) → capture off-table VIBDEPTH on `&0xFF` not the signed sum (C11), + relocated-state vibdepth code-overlap head (C6/C7-(b)); split by representability (vibdepth=static capture, wrap freq=per-subtune residue skip). **Per-round history (rounds 1–66) + every lever + methodology lessons live in project_dmc.md — READ IT FIRST.**
