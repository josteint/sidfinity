...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — `tools/fc_fingerprint.py` (reloc-invariant FC fingerprinting). 91% of HVSC FC (3673/4024) is ONE vanilla player → highest-leverage target. Migration in `pipelines/future_composer/standard/` (rep Carter/Jarre_2). The standard player is a DIFFERENT ENGINE from the Tel composer (pattern/instrument/effect/write-model all differ). DONE + gated (canaries 15/15): base + standard PATTERN decoder (commit 1af4a15; the "wrong notes" premise was a mis-diagnosis — pitches coincided, real fix was $Cx instrument-select) + instrument-select encoding fix + $40 effect (commit 3fdf1e9; = wave_arp REUSE, no new schema). V1 (inst4) wave program ctrl now matches orig EXACTLY. **RESUME: the $80 effect** (V3/inst7 — the biggest remaining lever, a 2-frame noise-click attack), then pulse/filter/wave-freq, then the base counter init-phase ($40 onset is 1 tick late = per-voice counter-phasing diff). Adrenalin engines are custom outliers (deprioritized).
