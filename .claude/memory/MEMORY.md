...
- [FC fingerprint DB + standard player](project_fc_fingerprint_and_standard.md) — reloc-invariant fingerprinting (`pipelines/future_composer/engine_fingerprint.py`); 91% of HVSC FC is ONE "standard" player. **2,572 FULL of 4,093 enumerated** ⚠STALE-VERDICTS since 2026-09-01 (backlog 31's key fix AND the same night's shared-plumbing edits to verify_cycle/batch_results; all byte-neutral — 6/6 byte-identical spot check). Full re-batch 2026-08-31, 0 regressions member-for-member. The former "2,604" counted 32 members #85 REMOVED (backlog 37); 47 dead pre-#85 rows remain in the store. Per-round detail in topic file — READ IT first.
...
- [Basic_Program family](project_basic_program.md) — 524 RSID-BASIC tunes on #85 (engine IS the BASIC+KERNAL ROM); trace-lift → USF round-trip. **489/524 FULL (93.3%)** ⚠STALE-VERDICTS since 2026-09-01 (backlog 31's portfolio-exclusion fix + the item-36 status stamp moved the key; both are byte-neutral, so the staleness is NOMINAL and unproven only because this family has no audit_rebuild binding). Last full re-batch 2026-08-31: 524/524 identical, 0 status changes, confirming the 2026-08-29 staleness was nominal. Earlier re-batch 2026-08-29, 0 regressions (the +1 is Cascading, the pw-sweep portfolio carrier, recovered at its honest 288s window — three stale caps, not a representation gap). Legion/Pepper: real divergences now, leads in backlog 14. Residue + round history: topic file.
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,758 SIDs on #85; largest family). **v4 f1 5,449/5,474 (99.5%) + f2 2,929/2,944 (99.5%)** — NOT closed; the old "100%" was a denominator artifact (50 claimed members had no verdict row → 37 partial + 3 error, and that IS the whole f1/f2 residue). Verdicts went ⚠STALE-VERDICTS 2026-08-29 16:40 (digi schema -> src/usf; 191/191 byte-identical over all 7 build paths, so the staleness is NOMINAL — backlog 31). Last full re-batch 2026-08-28, 0 regressions. **v5 GRIND OPEN: 1,211/2,031 FULL (59.6%)**, 820 partial, **0 unsupported / 0 error** — both `data_tables_off_image` refusals resolved (a real C26 unpacker + a misidentified player that got its own site map). Verdicts went ⚠STALE-VERDICTS again 2026-09-01 (backlog 31's key fix; byte-neutral, 6/6 byte-identical spot check). Full re-batch 2026-08-31 (2,078 members, 1h42m): 1211 full / 867 partial, **0 status changes and 0 first-divergence moves vs the pre-run rows**, i.e. the 2026-08-29 staleness was purely nominal (backlog 31). DMC-wide: 9,589/10,449 claimed = 91.8%, or 89.0% of the 10,774 corpus (309 unrouted + 16 v6 still uncovered). Table-overflow bucket CLOSED (C8 6th widening paged cursor + the owner-approved C11 live-position form `pulse_position`/`filter_position`); backlog 19 deleted. 2SID/3SID + compilations incl. heterogeneous ([[project_dmc_compilations]]). **READ project_dmc.md FIRST** (newest-first); do NOT expand this line.
...
- [Digi-Organizer](project_digi_organizer.md) — first parametric-digi family (`digi{}` schema, volume_4bit): **39/39 standalone FULL cycle-strict — family CLOSED**, ⚠STALE-VERDICTS since 2026-09-01 (backlog 31 derived this family's dependency closure, moving its key; 6/6 byte-identical, so nominal). 0 regressions; portfolio + regression wiring + mass-write done, artifacts re-synced 2026-08-31 after the universal-driver landing and verified 39/39 byte-identical from disk). Next: the 92 music-paired members, then Rayden_Digi. READ IT first.
- [Rayden_Digi](project_rayden_digi.md) — phase 3 of the digi parametrization; 17 self-driven RSID carriers. **Extract + ground-truth gate built**: 3 of 4 decoded members explain their ENTIRE $D418 stream. The sidid V1/V2 split is NOT a build fact (13 carriers share one sequencer). `SampleInstrument.loop_start` landed 2026-09-01 (owner-approved). **The remaining blocker is the MUSIC side, and it is 60% of the DMC f1 residue** — 15 of f1's 25 non-full members are Rayden carriers (backlog 40). READ IT first.
- [Music_Assembler](project_music_assembler_target.md) — **MIGRATED, SID→USF→SID** (3rd-largest, 6,489 SIDs on #85). **4,002 FULL of 6,462 enumerated (61.9%)** ⚠STALE-VERDICTS — the former "4,021" counted 19 members #85 REMOVED (27 dead pre-#85 rows remain in the store; `tools/verdict_gaps.py`, backlog 37). 16-member portfolio + the Freespace_2075 DMC+MA canary are tier 1; residue half CHIP-GLOBAL; mass-write deliberately NOT done. Surfaced ledger **C34**.
- [GoatTracker family](project_goattracker.md) — 2nd-largest (8,670). **Active: V1** (original 1.x, NOT GT2). Extract + composer built; wide batch **168/1387 FULL** ⚠STALE-VERDICTS since 2026-09-01. (Backlog 31's portfolio fix alone did NOT move this family — it has no portfolio file — but the same night's edits to `verify_cycle.py` and `batch_results.py`, both shared plumbing, moved EVERY family's key. Both are additive/reader-side, so builds are byte-identical and the staleness is nominal.) Full re-batch 2026-08-31: 1387/1387 identical, 0 status changes. Re-run THROUGH the stored `.usf`; wired into regression.py. Detail in topic file — READ IT first.
...
- [Chimera pipeline](project_chimera.md) — PSID rebuild (no KERNAL); music frame-exact; digi verified by a FLAT (reg,val) writeset, NOT cycle-strict (orig is RSID self-driven, rebuild PSID — cycle equality is structurally impossible; measured spectrally ~0% different, backlog 39). Drove the digi pipeline + no-verbatim-engine-bytes refactor. Includes the C64 banking gotcha.
