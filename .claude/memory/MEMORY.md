...
- [Basic_Program family](project_basic_program.md) — 524 RSID-BASIC tunes on #85 (engine IS the BASIC+KERNAL ROM); trace-lift → USF round-trip. **488/524 (93.1%) FULL, re-baselined 2026-08-12 under HONEST windows** (the 120s verify-window cap was a C20 eighth-layer false-FULL class: 44 exposed, 38 re-lifted FULL, 6 orphan-deleted; 31 measured songlength overrides — SLDB is broken for BASIC precalc; 13 gains + 38 new-#85 members swept same batch). Residue leads: fold-loopers lift (rescues image_too_big), pw-sweep portfolio gap, Polonaise ownership anomaly. NF adoption + spread-chord history: topic file.
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,758 SIDs in HVSC #85; largest family). **FAMILY-2 RESUMED 2026-08-12** (2,924 members on #85; fresh **2,884/2,924 FULL (98.6%), 0 unsupported** synced+audited (dual_phase +254, step_full +3, vibrato_ramp_persist +2); residue = 40 long-tail partials — see head entry). V4 f1: **5,401/5,401 FULL (100%)** (2026-08-04, r180 — Lane_Crazy closed via C8 pooled pulse-step layout + position-observability dedup key). Full 2SID/3SID (C27/C28) + compilations incl. HETEROGENEOUS ([[project_dmc_compilations]]). r164-166 closed Verdict/Verdict_01 FULL (the C19 "$FF→init RESTART, resume shape" wedge — a mid-song restart whose ghost frame garbage-indexes tables; solved in 3 layers: ghost-machinery restart burst + `glide_offtable` C6 redirect + `irecimg` 11-byte-record image for the garbage pulse-step read; re-test challenge disproved a "hard boundary" mis-claim 3×). **READ project_dmc.md FIRST** — round history + every lever + methodology live there (newest-first); do NOT expand this line (new rounds → topic file). Coverage VERIFIED 2026-08-11: fresh f1 batch **5,445/5,445 FULL** under the current code_hash, corpus mass-written + audited (10/10 from disk, all build paths) — batch rows, stored corpus, and code all agree; baseline tmp/dmc_f1_prev_batch.jsonl. The 2026-08-10/11 rounds (per-subtune vibdepth r183b, per-voice de-redirect r184-r186, canon-evidence gate r187) are family-wide validated by that batch.
