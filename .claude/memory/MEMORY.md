...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 **5359/5401 FULL (99.2%) + 42 partial + 0 error** (r129-r144 2026-07-30: wave in-table-runaway C2 +2, CIA cross-check/speed-mask/swing-median C9 +6, cymbal-ctrl/$FF-stub/transpose-bias wedges C19 +3, parity-wrapper P2 phase C24 +1, Deprave live-stack full overturn C29 +1, forced-record JSR-form wedge C19 +1, C37-observation resume wrapper +1, play-clock-in-sector C19 +1, C18 static-table refine +1, SID-mirror pw-base C19 +1, wavestep-arm C18 +2, $D418-every-play indirect-topology wrapper C19 +1 vs the r128c batch; corpus synced r128c — code_hash stale pending next fresh batch). Full 2SID/3SID (C27/C28) + compilation support incl. HETEROGENEOUS ([[project_dmc_compilations]]). **Round history + every lever + methodology live in project_dmc.md — READ IT FIRST** (do NOT expand this line; new rounds → topic file). Re-verify any stale wide batch before picking.
