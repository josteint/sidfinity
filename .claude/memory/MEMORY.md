...
- [DMC migration](project_dmc.md) — THE FOCUS ENGINE (10,676 SIDs; largest family). V4 f1 ~5378/5401 FULL (99.6%); residue = C19 singleton wedges + off-table/CIA hard residue. Full 2SID/3SID (C27/C28) + compilations incl. HETEROGENEOUS ([[project_dmc_compilations]]). r160 closed Vai/Hardtechno + Sad_End (filter step-DURATION table is an independently-relocated packer operand, not at op_filtdef+10 → resolve from the play code, NEW ledger **C39**, 2 members); r159 closed Bomberman_preview (CONDITIONAL per-subtune song remap → observed `subtune_songs` map, ledger **C19** 38th occ). **READ project_dmc.md FIRST** — round history + every lever + methodology live there (newest-first); do NOT expand this line (new rounds → topic file). Coverage = a FRESH batch (corpus code_hash stale pending next); re-verify any stale wide batch before picking.
