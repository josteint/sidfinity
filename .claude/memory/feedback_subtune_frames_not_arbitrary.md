...
description: "Verify with songlength × 1.1 frames (subtune_frames), never an arbitrary N. User has had to remind me multiple times."
metadata: 
  node_type: memory
  type: feedback
...
count is **songlength × 1.1 × 50 Hz** — never an arbitrary 500 / 1000
/ 2000. The Hubbard verify path uses
`pipelines.hubbard.verify.subtune_frames(config, passes=1.1)`; the
companion / new-engine paths get the same number by reading
`songlength_s` from `hvsc84.db` and computing
`round(1.1 * songlen * 50)`.
...
introduces false failures. The songlength × 1.1 window is exactly
...
subtune match for the full songlength × 1.1 window**. The skill
`migrate-hubbard-engine` already requires this; the rule applies to
all engines.

User has reminded me of this multiple times — last
nudge: *"why 500? Arbitrary! do subtunelength x 1.1! we have talked
