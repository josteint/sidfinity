"""Generate the universal das_model assembly (.s) for Commando, assemble via
xa65 into a PSID, and verify the rebuild against the original.

This is a DEBUG / documentation tool, not part of the production pipeline.
It exists so you can inspect Hubbard's engine semantics as plain readable
6502 assembly. The production pipeline goes through emit_usf.py and the
Lean codegen.
"""
import os
import sys
...
from pipelines.commando.extract.engine_model import extract as _extract_typed


def extract():
    """Legacy dict/tuple-shaped wrapper around the new dataclass extract.

    This demo file pre-dates the dataclass refactor and was written against the
    old `(T, instruments, score)` tuple of dicts. Rather than rewrite the
