...
- **For engine-state divergence, use `tools/state_diff.py` + auto-generated map.**
  ...
  - **When state_diff reports a divergence, always cross-check with `find_first_divergence.py` (writelog flat-prefix verdict).** If writelog matches, the state hint is noise.
