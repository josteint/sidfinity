...
- **For engine-state divergence, use `tools/state_diff.py` + auto-generated map.**
  - Build the map: `python3 pipelines/future_composer/state_map_gen.py --engine ENGINE --voice {1,2,3,all} --output MAP.py` — joins per-engine `pipelines/future_composer/<engine>/state_map.py` annotation with composer's xa65 labels. **DO NOT hand-craft state maps** (wrong addresses bit hard last session).
...
- **Specialised diagnostic tools** (full inventory + when-to-use in [`tools/INVESTIGATION_BACKLOG.md`](tools/INVESTIGATION_BACKLOG.md)):
  ...
  - `pipelines/future_composer/pattern_stream_decode.py --addr HEX [--seq]` — decode FC pattern/seq stream bytes as readable commands ($Cx wave/inst, $Fx markers, glide triples, etc.). Use when the bug is "what byte does the pattern stream actually have at this offset?"
  ...
  - `pipelines/future_composer/pattern_stream_verify.py --engine ENGINE` (or `--all`) — USF roundtrip check for the pattern-stream region. Verifies that orig and rebuild bytes match (accounting for `featuredriven_addr_shift` and the verbatim-pointer fixup). Catches data-emission regressions at extract/compose time.
...
- **Timestamped phase logging in dev tools (OWNER DIRECTIVE 2026-08-19).**
  Every development tool that can run more than a few seconds prints
  timestamped, FLUSHED phase lines via `src.tslog` (`ts()` one-liners,
  `phase()` start/end-with-elapsed banners): at minimum one line when a
  phase starts (naming its input scale — "capture 25 subtune pairs, 8
  workers") and one when it ends. Born after an evening of phantom hangs
  (dmc_build_one's 7.5-minute silent serial build phase killed as hung; a
  block-buffered redirect showing an empty log mid-run; a background run's
  completion mistaken for a 2-minute build). Wire it into any tool you
  touch that lacks it; new tools start with it.
...
python3 pipelines/dmc/build_one.py MUSICIANS/S/SilverFox/Seaside_99.sid --verify --localize
...
python3 pipelines/dmc/next_partial.py
...
python3 pipelines/dmc/smoke.py
...
**The `engine` / `engines` columns come from `tools/sidid_full.txt`**,
regenerated by **`python3 tools/gen_sidid_dump.py`** (~3 min) — never by hand,
because the three rules below used to live only in the output file's own header
where nothing enforced them. SIDId itself is vendored at `tools/sidid/` (cloned
+ patched + built by `tools/build.sh`).

- ⚠ **`-m` (multi-signature) is MANDATORY.** Without it sidid's loop breaks on
  the first hit, so the answer is whichever signature happens to come first in
  `sidid.cfg`. **48.2% of HVSC matches more than one player**, and the
  discarded remainder holds the two most useful facts: SUB-VERSIONS
  (`(DMC_V4.x)` 5,460 carriers vs `(DMC_V5.x)` 2,270 — sidid already knows v4
  from v5 — and the same shape for FC / Music_Assembler / Soundmonitor / JCH)
  and HETEROGENEOUS files (`Freespace_2075` →
  `DMC|(DMC_V4.x)|Music_Assembler|(Music_Assembler/MC)`, its real content).
  ⚠⚠ In `-m` output the 2nd and later matches of a file print with a **BLANK**
  name field, so a parser keyed on "every line starts with a path" drops them
  ALL silently and reports a tidy one-engine-per-file world. Parse via
  `tools/build_sid_db.load_engine_map`.
- ⚠ it scans EVERY `.sid`, so our own `.sidfinity.sid` rebuilds inside the tree
  get classified too and must be filtered out.
- ⚠ **upstream truncates paths to 56 chars for display** — fatal for a dump
  consumed as a `{path: engines}` map (it silently dropped 1,384 members, 2.3%
  of HVSC, which was 43% of everything reading as "unclassified").
  `tools/sidid_no_truncate.patch` removes it; a re-clone without the patch
  reintroduces the loss, so `gen_sidid_dump.py` FAILS if no long path survives.
  Also drop sidid's trailing per-player summary — those lines parse as bogus
  paths.

`engine` = the FIRST match (unchanged by the `-m` move — verified 0 differences
over all 59,826 classified files, so every existing query still works);
`engines` = all matches, `|`-separated. Query
`engines LIKE '%(DMC_V5.x)%'`; for a bare name use delimiters
(`'|' || engines || '|' LIKE '%|DMC|%'`) since names nest.
⚠ **sidid is a PRIOR, not an oracle** — byte-pattern matching, 41 of its 934
signatures are ≤8 bytes, it matches anywhere including DATA, and a PACKED file
matches the CRUNCHER not the player. Our own play-body dispatch is the oracle
where it exists (measured agreement on DMC v4-vs-v5: 7,429 vs 11 conflicts).
Use `engines` to GENERATE candidates for an unmigrated family or an unrouted
member, then let build+verify judge.
...
family's `pipelines/<family>/mass_write.py`). "What is stored" must equal "what was verified",
...
- **Audit from disk.** After writing, re-verify a sample **from the stored
  ...
  writer and verifier against each other. `pipelines/dmc/mass_write.py --audit N`
...
| After `tools/gen_sidid_dump.py` | refresh `engine` + `engines` columns |
