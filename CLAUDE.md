...
- **For engine-state divergence (orig's RAM byte vs rebuild's RAM byte at frame N), use `tools/state_diff.py` + `siddump --memwatch`.** libsidplayfp-accurate (NOT py65). The Python wrapper takes an orig↔rebuild address map and reports the first frame where any mapped pair differs. Use this whenever a divergence likely traces back to a state byte (pattern position, counter, flag) rather than a single effect's output write.
