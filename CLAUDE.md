...
4. **Convergence ledger reflex** ([`docs/convergence_ledger.md`](docs/convergence_ledger.md)) — three timings, don't conflate: (a) **CONSULT** before choosing how to solve ANY non-trivial problem — targeted lookup by problem-class in its Index; if there's an entry, use its solution instead of inventing variant #15. (b) **RECORD** every solution to a non-trivial problem on first sight (status `logged`) — even a 1× one, so the next occurrence is a lookup not a memory feat. (c) **CANONICALIZE** on the 2nd occurrence (≥2× → pick the one form / `factor-candidate`). The ≥2× threshold gates only canonicalize/factor, never whether it's recorded. It's a record that pre-decides Move-1 convergence, not a refactor; `/uready-review` is the periodic maintainer.
